...
public class StatisticsPrinter implements Callable<Void> {
	...
	static double total_inits, total_balances, total_bandwagons, total_joins, init_myCap_min, init_myCap_med,
			...
			join_capRatio_min, join_capRatio_med, join_capMed_min, join_capMed_med, join_capStd_min, join_capStd_med,
	...
	private static void calculateProfileVariables() {
		...
		if (currentLevel > 2) {
			...
			if (total_joins > 0) {
				...
				double[] join_capStd = new double[totalJoins];
				...
				for (int l = 0; l < PROFILES_PER_CAT; l++) {
					if (joiningProfiles[currentDeme][l] == true) {
						...
						join_capStd[totalJoins - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].capStd;
				...
				if (!even) {
					...
					join_capStd_med = join_capStd[(int) total_joins / 2];
					...
					join_capStd_med = (join_capStd[(int) total_joins / 2] + join_capStd[(int) (total_joins / 2) - 1]) / 2;
	...
	private static void printProfileVariablesToFile() {
		...
		if (currentLevel > 2) {
			outputString += ("," + balance_myCap_min + "," + balance_myCap_med + "," + balance_oppCap_min + ","
					...
					+ bandwagon_capMed_min + "," + bandwagon_capMed_med + "," + bandwagon_capStd_min + ","
					+ bandwagon_capStd_med + "," + bandwagon_capMin_min + "," + bandwagon_capMin_med + ","
					...
					+ bandwagon_smallerSideLeaderCap_med + "," + bandwagon_leftCapSum_min + ","
					...
					+ join_capRatio_med + "," + join_capMed_min + "," + join_capMed_med + "," + join_capStd_min + ","
					+ join_capStd_med + "," + join_capMin_min + "," + join_capMin_med + "," + join_capMax_min + ","
					...
					+ join_smallerSideLeaderCap_med + "," + join_leftCapSum_min + "," + join_leftCapSum_med + ","
					+ join_myEnmity_min + "," + join_myEnmity_med + "," + join_oppEnmity_min + "," + join_oppEnmity_med);
		}
	...
	private static void printProfileWorlds() {
		...
		try {
			...
			outputString = "generation" + "," + "profile_no" + "," + "simulate_Joiner" + "," + "world_size" + ","
					+ "capMed" + "," + "capStd" + "," + "capMin" + "," + "capMax" + "," + "myCap" + ","
					+ "oppCap" + "," + "capRatio" + "," + "smallerSideCap" + ","
			...
			if (i > 0)
				for (int k = 0; k < MasterVariables.PROFILES_PER_CAT; k++) {
					outputString += (currentGen + "," + (k + 1) + "," + "1" + "," + (i + MasterVariables.MINSYSTEM)
							...
							+ MasterVariables.totalProfileBandwagons[i - 1][k] + "," + MasterVariables.totalProfileJoins[i - 1][k]);
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
						outputString += ("," + ".");
					}
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
