...
public class MainClass {
	...
	private static void runSimulation() {
		...
		for (int worldSize = 2; worldSize <= maxWorldSize; worldSize++) {
			...
			int[][] resourcesDistribution = DG.calcDistribution(worldSize, resourcesStepSize, 100, false, true);
			int[][] territoriesDistribution = new int[1][];
			if (!terrDistEqualResDest){
				territoriesDistribution = DG.calcDistribution(worldSize, territoryStepSize, 100, false, true);
			}
			for (int attackOrder = 0; attackOrder < worldSize; attackOrder++) {
				for (int resDistCounter = 0; resDistCounter < resourcesDistribution.length; resDistCounter++) {
					...
					double EEV = calcEEV(attackOrder, resourcesDistribution[resDistCounter], territoriesDistribution[terrDistCounter], sampleSize);
					results.add(new Result(worldSize, attackOrder, EEV, resourcesDistribution[resDistCounter], resourcesDistribution[resDistCounter]));
					}
				}
			}
		}
	...
	private static double calcEEV(int attackOrder, int[] resDistr, int[] terrDistr, int sampleSize) {
		double EEV = 0;
		RandomWorld RW = new RandomWorld(attackOrder, resDistr, terrDistr);
