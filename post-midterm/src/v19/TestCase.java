...
public class TestCase {
	...
	public TestCase(double[] capabilities, double[][] capChangeRates, double[] randNums, double[] randGauss,
			int testedStrategyIndex, double init_proness, double balance_proness, double join_proness, Strategy[] strategies, boolean isInit) {
	...
	void resetTestCase(Strategy testedStrategy) {
		strategies[testedStrategyIndex] = testedStrategy;
		...
		findJoiners: do {
			...
			for (int i = 0; i < initialSystemSize; i++) {
				...
				if (i == testedStrategyIndex) {
					char[][] joinStrategy = strategies[i].getJoinStrategy(currentSystemSize);

					leftCapSum = 1 - sideACap - sideBCap - myCap;
					if (Math.abs(leftCapSum) < 1E-10)
						leftCapSum = 0;
					output = runJoin(joinStrategy);
				}
				else{
					double x= (double) 1/3;
					double comp=randNums[randNumCounter++];
					if (comp<x)
					output=0;
					else if (comp<x*2)
						output=1;
	...
	private boolean findInitators() {
		...
		for (int i = 0; i < initialSystemSize; i++) {
			if (surviving[i]) {
				...
				if (i==testedStrategyIndex){
				initStrategy = strategies[i].getInitStrategy(currentSystemSize);
				

				myCap = capabilities[i];
				targetOrder = runInit(initStrategy);
				}
				if (i!=testedStrategyIndex){
					if (currentSystemSize == 3) {
						double x = (double) 1 / 3;
						double comp = randNums[randNumCounter++];
						if (comp < x)
							targetOrder = 0;
	...
	void simulateJoiners() {
		...
		int attackerIndex = i++;
		warringB[attackerIndex] = true;

		if (i == testedStrategyIndex)
			i++;
		int targetIndex = i;
		warringA[targetIndex] = true;
		sideBCap = capabilities[attackerIndex];
		sideACap = capabilities[targetIndex];
		leftCapSum = 1 - sideACap - sideBCap;
		if (Math.abs(leftCapSum) < 1E-10)
			leftCapSum = 0;
