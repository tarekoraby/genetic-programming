...
public class StatisticsPrinter implements Callable<Void> {
	...
	static Strategy bestInitStrategy, bestJoinStrategy;
	...
	static int initFitness, joinFitness;
	// xxx
	static double[] currTotBal = new double[3];
	static double[] prevTotBal = new double[3];
	static boolean printANDclose = false;

	StatisticsPrinter(Strategy bestInitStrategy, Strategy bestJoinStrategy, int initFitness, int joinFitness,
			int currentSimulation, int currentGen, int currentLevel, int currentDeme) {
		if (printInProgress) {
			System.out.println("Error StatisticsPrinter class !!! Print in progress");
			System.exit(0);
		}
		printInProgress = true;
		StatisticsPrinter.bestInitStrategy = bestInitStrategy;
		StatisticsPrinter.bestJoinStrategy = bestJoinStrategy;
		...
		StatisticsPrinter.initFitness = initFitness;
		StatisticsPrinter.joinFitness = joinFitness;
		...
		if (currentLevel > 2) {
			balanceProfiles = MasterVariables.balanceProfiles[currentLevel - 3];
			bandwagonProfiles = MasterVariables.bandwagonProfiles[currentLevel - 3];
			joiningProfiles = MasterVariables.joiningProfiles[currentLevel - 3];
		}
		checkErrors();
	}
	...
	private static void populateProfiles() {
		...
		for (int i = 0; i < PROFILES_PER_CAT; i++) {
			attackProfiles[currentDeme][i] = profilingWorlds_Attacks[currentLevel - 2][i].willItAttack(bestInitStrategy
		...
		if (currentLevel > 2)
			for (int i = 0; i < PROFILES_PER_CAT; i++) {
				joiningProfiles[currentDeme][i] = profilingWorlds_Joins[currentLevel - 3][i]
						.willItAttack(bestJoinStrategy.getJoinStrategy(currentLevel));
				if (joiningProfiles[currentDeme][i]) {
					MasterVariables.totalProfileJoins[currentSimulation - 1][currentLevel - 3][i]++;
					// System.out.println(currentSimulation + " " + currentLevel
					// + " " + currentDeme + " " + i);
					...
					balanceProfiles[currentDeme][i] = profilingWorlds_Joins[currentLevel - 3][i]
							.willItAttack(bestJoinStrategy.getBalanceStrategy(currentLevel));
	...
	private static void printProfileVariablesToFile() {
		...
		MasterVariables.state_profiles_writer.writeToFile(currentSimulation + ","
				+ MasterVariables.GENERATIONS_PER_DEME + "," + MasterVariables.RANDOMPLAYER_PROB + ","
				+ MasterVariables.INTERACTIONROUNDS + "," + MasterVariables.RANDOMNUMBERS + "," + stateNumber + ","
				+ currentGen + "," + currentLevel + "," + total_inits + ",");
		if (currentLevel == 2)
			MasterVariables.state_profiles_writer.writeToFile("." + "," + "." + "," + ".");
		else
			MasterVariables.state_profiles_writer.writeToFile(total_balances + "," + total_bandwagons + ","
					+ total_joins);

		MasterVariables.state_profiles_writer.writeToFile("," + initFitness + "," + joinFitness);
		...
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			if (j > currentLevel || j == stateNumber)
				...
				MasterVariables.state_profiles_writer.writeToFile(","
						+ calcSimilarity(attackProfiles[currentDeme], attackProfiles[j - 1]));
		}
		...
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			if (j > currentLevel || j == stateNumber || currentLevel == 2)
				...
				MasterVariables.state_profiles_writer.writeToFile(","
						+ calcSimilarity(balanceProfiles[currentDeme], balanceProfiles[j - 1]));
		}
		...
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			if (j > currentLevel || j == stateNumber || currentLevel == 2)
				...
				MasterVariables.state_profiles_writer.writeToFile(","
						+ calcSimilarity(bandwagonProfiles[currentDeme], bandwagonProfiles[j - 1]));
		}
		...
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			if (j > currentLevel || j == stateNumber || currentLevel == 2)
				...
				MasterVariables.state_profiles_writer.writeToFile(","
						+ calcSimilarity(joiningProfiles[currentDeme], joiningProfiles[j - 1]));
		}

		MasterVariables.state_profiles_writer.writeToFile("," + calcGenSimilarity(attackProfiles));

		if (currentLevel == 2)
			MasterVariables.state_profiles_writer.writeToFile("," + "." + "," + "." + "," + ".");
		else {
			MasterVariables.state_profiles_writer.writeToFile("," + calcGenSimilarity(balanceProfiles) + ","
					+ calcGenSimilarity(bandwagonProfiles) + "," + calcGenSimilarity(joiningProfiles));
		}

		MasterVariables.state_profiles_writer.writeToFile("," + init_myCap_min + "," + init_myCap_med + ","
				+ init_oppCap_min + "," + init_oppCap_med + "," + init_capRatio_min + "," + init_capRatio_med + ","
				+ init_capMed_min + "," + init_capMed_med + "," + init_capSD_min + "," + init_capSD_med + ","
				+ init_cap_1_min + "," + init_cap_1_med + "," + init_cap_2_min + "," + init_cap_2_med + ","
				+ init_cap_3_min + "," + init_cap_3_med + "," + init_cap_4_min + "," + init_cap_4_med + ","
				+ init_cap_5_min + "," + init_cap_5_med + "," + init_cap_6_min + "," + init_cap_6_med + ","
				+ init_cap_7_min + "," + init_cap_7_med + "," + init_cap_8_min + "," + init_cap_8_med + ","
				+ init_smallerSideCap_min + "," + init_smallerSideCap_med + "," + init_largerSideCap_min + ","
				+ init_largerSideCap_med + "," + init_smallerSideLeaderCap_min + "," + init_smallerSideLeaderCap_med
				+ "," + init_largerSideLeaderCap_min + "," + init_largerSideLeaderCap_med + "," + init_leftCapSum_min
				+ "," + init_leftCapSum_med + "," + init_myEnmity_min + "," + init_myEnmity_med + ","
				+ init_oppEnmity_min + "," + init_oppEnmity_med);

		if (currentLevel > 2) {
			MasterVariables.state_profiles_writer.writeToFile("," + balance_myCap_min + "," + balance_myCap_med + ","
					+ balance_oppCap_min + "," + balance_oppCap_med + "," + balance_capRatio_min + ","
					+ balance_capRatio_med + "," + balance_capMed_min + "," + balance_capMed_med + ","
					+ balance_capSD_min + "," + balance_capSD_med + "," + balance_cap_1_min + "," + balance_cap_1_med
					+ "," + balance_cap_2_min + "," + balance_cap_2_med + "," + balance_cap_3_min + ","
					+ balance_cap_3_med + "," + balance_cap_4_min + "," + balance_cap_4_med + "," + balance_cap_5_min
					+ "," + balance_cap_5_med + "," + balance_cap_6_min + "," + balance_cap_6_med + ","
					+ balance_cap_7_min + "," + balance_cap_7_med + "," + balance_cap_8_min + "," + balance_cap_8_med
	...
	private static void printProfileWorlds() {
		...
		if (currentLevel == 3) {
			currTotBal[currentDeme] = total_balances;
			if (currentGen > 100
					& (currTotBal[0] != prevTotBal[0] || currTotBal[1] != prevTotBal[1] || currTotBal[2] != prevTotBal[2]))
				printANDclose = true;
			else {
				prevTotBal[currentDeme] = currTotBal[currentDeme];
			}
		}

		if ((!printANDclose && !(currentLevel == MasterVariables.MAXSYSTEM && currentDeme == currentLevel - 1))
				|| (currentGen % 50 != 0 || !(currentLevel == MasterVariables.MAXSYSTEM && currentDeme == currentLevel - 1)))
			return;
		printANDclose = false;
		try {
		...
		MasterVariables.profiles_count_writer
				.writeToFile("simulation" + "," + "generation" + "," + "genPerSimulation" + "," + "profile_no" + ","
						...
						+ "largerSideCap" + "," + "smallerSideLeaderCap" + "," + "largerSideLeaderCap" + ","
						+ "leftCapSum" + "," + "myEnmity" + "," + "oppEnmity" + "," + "Simulation_totProfAttacks" + ","
						+ "Simulation_totProfBalances" + "," + "Simulation_totProfBandwagons" + ","
						+ "Simulation_totProfJoins");
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			MasterVariables.profiles_count_writer.writeToFile("," + "totalProfileAttack_state_" + j);
		}
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			MasterVariables.profiles_count_writer.writeToFile("," + "totalProfileBalances_state_" + j);
		}
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			MasterVariables.profiles_count_writer.writeToFile("," + "totalProfileBandwagons_state_" + j);
		}
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			MasterVariables.profiles_count_writer.writeToFile("," + "totalProfileJoins_state_" + j);
		}

		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			MasterVariables.profiles_count_writer.writeToFile("," + "attack_state_" + j);
		}
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			MasterVariables.profiles_count_writer.writeToFile("," + "balance_state_" + j);
		}
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			MasterVariables.profiles_count_writer.writeToFile("," + "bandwagon_state_" + j);
		}
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			MasterVariables.profiles_count_writer.writeToFile("," + "join_state_" + j);
		}
			...
			for (int i = 0; i < MasterVariables.uniqueWorldSizes; i++) {
				for (int k = 0; k < MasterVariables.PROFILES_PER_CAT; k++) {
					MasterVariables.profiles_count_writer.writeToFile(Integer.toString(simulationIndex + 1) + ","
							+ currentGen + "," + MasterVariables.MASTER_GENERATIONS + "," + (k + 1) + "," + "0" + ","
					...
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
						if (j <= i + 1) {
							MasterVariables.profiles_count_writer.writeToFile(","
									+ MasterVariables.totalProfileAttacks_PerState[simulationIndex][i][j][k]);
						} else
							MasterVariables.profiles_count_writer.writeToFile("," + ".");
					}
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
					...
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
						if (j <= i + 1) {
							if (MasterVariables.attackProfiles[i][j][k])
								MasterVariables.profiles_count_writer.writeToFile("," + "1");
							else
								MasterVariables.profiles_count_writer.writeToFile("," + "0");
						} else
							MasterVariables.profiles_count_writer.writeToFile("," + ".");
					}
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
				...
				if (i > 0)
					for (int k = 0; k < MasterVariables.PROFILES_PER_CAT; k++) {
						MasterVariables.profiles_count_writer.writeToFile(Integer.toString(simulationIndex + 1) + ","
								+ currentGen + "," + MasterVariables.MASTER_GENERATIONS + "," + (k + 1) + "," + "1"
								+ "," + (i + MasterVariables.MINSYSTEM) + ","
						...
						for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
							if (j <= i + 1) {
								MasterVariables.profiles_count_writer.writeToFile(","
										+ MasterVariables.totalProfileBalances_PerState[simulationIndex][i - 1][j][k]);
							} else
								MasterVariables.profiles_count_writer.writeToFile("," + ".");
						}
						for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
							if (j <= i + 1) {
								MasterVariables.profiles_count_writer
										.writeToFile(","
												+ MasterVariables.totalProfileBandwagons_PerState[simulationIndex][i - 1][j][k]);
							} else
								MasterVariables.profiles_count_writer.writeToFile("," + ".");
						}
						for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
							if (j <= i + 1) {
								MasterVariables.profiles_count_writer.writeToFile(","
										+ MasterVariables.totalProfileJoins_PerState[simulationIndex][i - 1][j][k]);
							} else
								MasterVariables.profiles_count_writer.writeToFile("," + ".");
						}
						...
						for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
							if (j <= i + 1) {
								if (MasterVariables.balanceProfiles[i - 1][j][k])
									MasterVariables.profiles_count_writer.writeToFile("," + "1");
								else
									MasterVariables.profiles_count_writer.writeToFile("," + "0");
							} else
						...
						for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
							if (j <= i + 1) {
								if (MasterVariables.bandwagonProfiles[i - 1][j][k])
									MasterVariables.profiles_count_writer.writeToFile("," + "1");
								else
									MasterVariables.profiles_count_writer.writeToFile("," + "0");
							} else
						...
						for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
							if (j <= i + 1) {
								if (MasterVariables.joiningProfiles[i - 1][j][k])
									MasterVariables.profiles_count_writer.writeToFile("," + "1");
								else
									MasterVariables.profiles_count_writer.writeToFile("," + "0");
							} else
