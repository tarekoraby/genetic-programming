...
public class TestCase {
	...
	public TestCase(double[] capabilities, int[][] capChangeRates, double[] randNums, double[] randGauss,
	...
	int simulate() {
		...
		do {
			if (currentSystemSize != initialSystemSize) {
				warInitiated = findLowerInitators();
				if (warInitiated) {
				...
				if (MasterVariables.testInit == false) {
					warInitiated = true;
					newMethod();
					findJoiners();
					resolveWar();
		...
		/*do {
			if (MasterVariables.testInit == true || currentSystemSize != initialSystemSize) {
				warInitiated = findInitators();
				if (warInitiated) {
					if (currentSystemSize > 2)
						findJoiners();
	...
	private void findLowerJoiners() {
		...
		findJoiners: do {
			...
			for (int i = 0; i < initialSystemSize; i++) {
				if (surviving[i]) {
					demeCounter++;
					if (demeCounter == currentSystemSize)
						demeCounter = 0;
				}
				...
				if (randNums[randNumCounter++] < MasterVariables.RANDOMPLAYER_PROB) {
					...
					oppCap = 1;
					strategy = MasterVariables.bestJoinStrategies[currentSystemSize - 3][demeCounter]
								.getJoinStrategy(currentSystemSize);
					...
					if (run() == 1) {
						PC = 0;
						strategy = MasterVariables.bestJoinStrategies[currentSystemSize - 3][demeCounter]
									.getBalanceStrategy(currentSystemSize);
						if (run() == 1) {
							if (sideACap < sideBCap) {
								sideACap += myCap;
								warringA[i] = true;
	...
	private boolean findLowerInitators() {
		...
		for (int i = 0; i < initialSystemSize; i++) {
			if (surviving[i]) {
				...
				if (randNums[randNumCounter++] < MasterVariables.RANDOMPLAYER_PROB) {
					...
					myCap = capabilities[i];
					strategy = MasterVariables.bestInitStrategies[currentSystemSize - 2][demeCounter]
							.getInitStrategy(currentSystemSize);
	...
	private void findJoiners() {
		...
		findJoiners: do {
			...
			for (int i = 0; i < initialSystemSize; i++) {
				if (surviving[i])
					demeCounter++;
				if (demeCounter == MasterVariables.testedDeme && currentSystemSize == initialSystemSize)
					demeCounter++;
				if (demeCounter == currentSystemSize)
					demeCounter = 0;
				if (!surviving[i] || warringA[i] || warringB[i])
					continue;
				checkRandNumCounter();
				if ((i != testedStrategyIndex || currentSystemSize != initialSystemSize || MasterVariables.testInit)
						&& randNums[randNumCounter++] < MasterVariables.RANDOMPLAYER_PROB) {
					if (randNums[randNumCounter++] < randNums[randNumCounter++] * join_proness) {
						if (randNums[randNumCounter++] < randNums[randNumCounter++] * balance_proness) {
							if (sideACap < sideBCap) {
								sideACap += capabilities[i];
					...
					myCap = capabilities[i];
					oppCap = 1;
					if (i == testedStrategyIndex && currentSystemSize == MasterVariables.testedSize
							&& !MasterVariables.testInit)
	...
	private boolean findInitators() {
		...
		for (int i = 0; i < initialSystemSize; i++) {
			if (surviving[i]) {
				...
				if ((i != testedStrategyIndex  || initialSystemSize != currentSystemSize || !MasterVariables.testInit)
						&& randNums[randNumCounter++] < MasterVariables.RANDOMPLAYER_PROB) {
					if (randNums[randNumCounter++] < randNums[randNumCounter++] * init_proness) {
						warringA = new boolean[initialSystemSize];
						warringB = new boolean[initialSystemSize];
					...
					myCap = capabilities[i];
					if (i == testedStrategyIndex && currentSystemSize == MasterVariables.testedSize && MasterVariables.testInit)
						strategy = testedInitStrategy;
					else
						strategy=MasterVariables.bestInitStrategies[currentSystemSize - 2][demeCounter].getInitStrategy(currentSystemSize);
					/*int k = i, counterToOpp = opponentDistance;
