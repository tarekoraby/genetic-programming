...
public class TestCase {
	...
	public double capMed, capStd, capMin, capMax, myCap, oppCap, mySideCap, leftCapSum, myEnmity, oppEnmity;
	...
	public TestCase(double[] capabilities, double[][] capChangeRates, double[] randNums, double[] randGauss, int testedStrategyIndex) {
		...
		this.original_capabilities = (double[]) capabilities.clone();
	...
	void resetTestCase(Strategy[] strategies) {
		...
		capabilities = (double[]) original_capabilities.clone();
		LeftInteractionRounds = MasterVariables.INTERACTIONROUNDS;
			...
			for (int i = 0; i < initialSystemSize; i++) {
				...
				if (randNums[randNumCounter++] < MasterVariables.RANDOMPLAYER_PROB) {
					...
					case 0:
						...
						strategy = strategies[i].getBandwagonStrategy(currentSystemSize);
						PC = 0;
						if (run() == 1) {
							if (sideASmaller) {
								sideB_cap += myCap;
