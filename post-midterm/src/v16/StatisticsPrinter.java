		...
		for (int profCounter = 0; profCounter < PROFILES_PER_CAT; profCounter++) {
			if (profilingWorlds_Attacks[0][profCounter].myCap == profilingWorlds_Attacks[0][profCounter].cap_2)
				for (int strategyCounter = 0; strategyCounter < populationAttack_2[0].length; strategyCounter++) {
					init = profilingWorlds_Attacks[0][profCounter].willItAttack(populationAttack_2[0][strategyCounter]
							.getInitStrategy(2));
					if (init) {
						total_inits++;
		...
		for (int profCounter = 0; profCounter < PROFILES_PER_CAT; profCounter++) {
			if (profilingWorlds_Attacks[0][profCounter].myCap == profilingWorlds_Attacks[0][profCounter].cap_1)
				for (int strategyCounter = 0; strategyCounter < populationAttack_2[1].length; strategyCounter++) {
					init = profilingWorlds_Attacks[0][profCounter].willItAttack(populationAttack_2[1][strategyCounter]
							.getInitStrategy(2));
					if (init) {
						total_inits++;
						MasterVariables.totalProfileAttacks[currentSimulation - 1][0][profCounter]++;
				...
				if (profilingWorlds_Joins[0][profCounter].cap_1 < 0.5) {
					if (profilingWorlds_Joins[0][profCounter].myCap == profilingWorlds_Joins[0][profCounter].cap_3) {						
						for (int strategyCounter = 0; strategyCounter < populationJoin_3_1[0].length; strategyCounter++) {
							join = profilingWorlds_Joins[0][profCounter].willItAttack(populationJoin_3_1[0][strategyCounter]
									.getJoinStrategy(3));
							if (join) {
								total_joins++;
								...
								bal = profilingWorlds_Joins[0][profCounter]
										.willItAttack(populationJoin_3_1[0][strategyCounter].getBalanceStrategy(3));
								if (bal) {
									total_balances++;
						...
						for (int strategyCounter = 0; strategyCounter < populationJoin_3_1[1].length; strategyCounter++) {
							join = profilingWorlds_Joins[0][profCounter].willItAttack(populationJoin_3_1[1][strategyCounter]
									.getJoinStrategy(3));
							if (join) {
								total_joins++;
								MasterVariables.totalProfileJoins[currentSimulation - 1][0][profCounter]++;
								...
								bal = profilingWorlds_Joins[0][profCounter]
										.willItAttack(populationJoin_3_1[1][strategyCounter].getBalanceStrategy(3));
								if (bal) {
									total_balances++;
									MasterVariables.totalProfileBalances[currentSimulation - 1][0][profCounter]++;
								} else {
						...
						for (int strategyCounter = 0; strategyCounter < populationJoin_3_1[2].length; strategyCounter++) {
							join = profilingWorlds_Joins[0][profCounter].willItAttack(populationJoin_3_1[2][strategyCounter]
									.getJoinStrategy(3));
							if (join) {
								total_joins++;
								MasterVariables.totalProfileJoins[currentSimulation - 1][0][profCounter]++;
								...
								bal = profilingWorlds_Joins[0][profCounter]
										.willItAttack(populationJoin_3_1[2][strategyCounter].getBalanceStrategy(3));
								if (bal) {
									total_balances++;
									MasterVariables.totalProfileBalances[currentSimulation - 1][0][profCounter]++;
								} else {
						...
						if (profilingWorlds_Joins[0][profCounter].myCap == profilingWorlds_Joins[0][profCounter].cap_3) {
							for (int strategyCounter = 0; strategyCounter < populationJoin_3_2[0].length; strategyCounter++) {
								join = profilingWorlds_Joins[0][profCounter].willItAttack(populationJoin_3_2[0][strategyCounter]
										.getJoinStrategy(3));
								if (join) {
									total_joins++;
									MasterVariables.totalProfileJoins[currentSimulation - 1][0][profCounter]++;
									...
									bal = profilingWorlds_Joins[0][profCounter]
											.willItAttack(populationJoin_3_2[0][strategyCounter].getBalanceStrategy(3));
									if (bal) {
										total_balances++;
										MasterVariables.totalProfileBalances[currentSimulation - 1][0][profCounter]++;
									} else {
							...
							for (int strategyCounter = 0; strategyCounter < populationJoin_3_2[1].length; strategyCounter++) {
								join = profilingWorlds_Joins[0][profCounter].willItAttack(populationJoin_3_2[1][strategyCounter]
										.getJoinStrategy(3));
								if (join) {
									total_joins++;
									MasterVariables.totalProfileJoins[currentSimulation - 1][0][profCounter]++;
									...
									bal = profilingWorlds_Joins[0][profCounter]
											.willItAttack(populationJoin_3_2[1][strategyCounter].getBalanceStrategy(3));
									if (bal) {
										total_balances++;
										MasterVariables.totalProfileBalances[currentSimulation - 1][0][profCounter]++;
									} else {
							...
							for (int strategyCounter = 0; strategyCounter < populationJoin_3_2[2].length; strategyCounter++) {
								join = profilingWorlds_Joins[0][profCounter].willItAttack(populationJoin_3_2[2][strategyCounter]
										.getJoinStrategy(3));
								if (join) {
									total_joins++;
									MasterVariables.totalProfileJoins[currentSimulation - 1][0][profCounter]++;
									...
									bal = profilingWorlds_Joins[0][profCounter]
											.willItAttack(populationJoin_3_2[2][strategyCounter].getBalanceStrategy(3));
									if (bal) {
										total_balances++;
										MasterVariables.totalProfileBalances[currentSimulation - 1][0][profCounter]++;
									} else {
				...
				if (profilingWorlds_Attacks[1][profCounter].cap_1 < 0.5) {
					if (profilingWorlds_Attacks[1][profCounter].myCap == profilingWorlds_Attacks[1][profCounter].cap_3) {
						for (int strategyCounter = 0; strategyCounter < populationAttack_3_1[0].length; strategyCounter++) {
							init = profilingWorlds_Attacks[1][profCounter].willItAttack(populationAttack_3_1[0][strategyCounter]
									.getInitStrategy(3));
							if (init) {
								total_inits++;
						...
						for (int strategyCounter = 0; strategyCounter < populationAttack_3_1[1].length; strategyCounter++) {
							init = profilingWorlds_Attacks[1][profCounter].willItAttack(populationAttack_3_1[1][strategyCounter]
									.getInitStrategy(3));
							if (init) {
								total_inits++;
								MasterVariables.totalProfileAttacks[currentSimulation - 1][1][profCounter]++;
						...
						for (int strategyCounter = 0; strategyCounter < populationAttack_3_1[2].length; strategyCounter++) {
							init = profilingWorlds_Attacks[1][profCounter].willItAttack(populationAttack_3_1[2][strategyCounter]
									.getInitStrategy(3));
							if (init) {
								total_inits++;
								MasterVariables.totalProfileAttacks[currentSimulation - 1][1][profCounter]++;
						...
						if (profilingWorlds_Attacks[1][profCounter].myCap == profilingWorlds_Attacks[1][profCounter].cap_3) {
							for (int strategyCounter = 0; strategyCounter < populationAttack_3_2[0].length; strategyCounter++) {
								init = profilingWorlds_Attacks[1][profCounter].willItAttack(populationAttack_3_2[0][strategyCounter]
										.getInitStrategy(3));
								if (init) {
									total_inits++;
									MasterVariables.totalProfileAttacks[currentSimulation - 1][1][profCounter]++;
							...
							for (int strategyCounter = 0; strategyCounter < populationAttack_3_2[1].length; strategyCounter++) {
								init = profilingWorlds_Attacks[1][profCounter].willItAttack(populationAttack_3_2[1][strategyCounter]
										.getInitStrategy(3));
								if (init) {
									total_inits++;
									MasterVariables.totalProfileAttacks[currentSimulation - 1][1][profCounter]++;
							...
							for (int strategyCounter = 0; strategyCounter < populationAttack_3_2[2].length; strategyCounter++) {
								init = profilingWorlds_Attacks[1][profCounter].willItAttack(populationAttack_3_2[2][strategyCounter]
										.getInitStrategy(3));
								if (init) {
									total_inits++;
									MasterVariables.totalProfileAttacks[currentSimulation - 1][1][profCounter]++;
