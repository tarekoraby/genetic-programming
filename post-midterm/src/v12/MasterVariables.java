...
public final class MasterVariables {
	...
	///xxxx final
	static final int MASTER_GENERATIONS = 10000, POPSIZE_PER_PROCESSOR = 25, MINRANDOM = 0, MAXRANDOM = 1000,
			PROFILES_PER_CAT = 1000, MINSYSTEM = 2, MAXSYSTEM = 3, MAX_LEN = 10000, DEPTH = 5, TSIZE = 2,
			simulations = 2, GENERATIONS_PER_DEME = 1;

	// /xxxx final
	static final int RANDOMNUMBERS = 20, CAPMED = RANDOMNUMBERS, CAPSD = CAPMED + 1, CAP_1 = CAPSD + 1,
			...
			FSET_3_END = OR;
	///xxxx final
	static double RANDOMPLAYER_PROB = 0.01, ENHANCMENT_MARGIN = 0.01, PMUT_PER_NODE = 0, MOD_CROSSOVER_PROB = 0.2,
			REPLICATION_PROB = 0.2, CROSSOVER_PROB = 0.2, SUBTREE_MUT_PROB = 0.2, PMUT_PROB = 0.2,
			ABIOGENSIS_PROB = 0, GAUSS_PROB_FACTOR = 0;
	///xxxx final
	...
	private void initializeFitnessWorlds() {
		...
		for (int k = 0; k < uniqueWorldSizes; k++) {
			...
			for (int test = 0; test < TESTCASES[k]; test++) {
				...
				masterInitProness[k][test] = rd.nextDouble() * 2;
				masterBalanceProness[k][test] = rd.nextDouble() * 2;
				masterJoinProness[k][test] = rd.nextDouble() * 2;
			}
		}
	}
