...
public class StatisticsPrinter implements Callable<Void> {
	...
	private static void printProfileVariablesToFile() {
		...
		MasterVariables.state_profiles_writer.writeToFile(currentSimulation + "," + MasterVariables.GENERATIONS_PER_DEME + ","
		...
		if (currentLevel == 2)
			...
			MasterVariables.state_profiles_writer.writeToFile(total_balances + "," + total_bandwagons + "," + total_joins);

		MasterVariables.state_profiles_writer.writeToFile("," + fitness);

		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			if (j > currentLevel || j == stateNumber)
				MasterVariables.state_profiles_writer.writeToFile("," + ".");
			else
				MasterVariables.state_profiles_writer.writeToFile("," + calcSimilarity(attackProfiles[currentDeme], attackProfiles[j - 1]));
		}
		...
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			if (j > currentLevel || j == stateNumber || currentLevel == 2)
				...
				MasterVariables.state_profiles_writer.writeToFile("," + calcSimilarity(balanceProfiles[currentDeme], balanceProfiles[j - 1]));
		}
		...
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			if (j > currentLevel || j == stateNumber || currentLevel == 2)
				...
				MasterVariables.state_profiles_writer.writeToFile("," + calcSimilarity(bandwagonProfiles[currentDeme], bandwagonProfiles[j - 1]));
		}
		...
		for (int j = 1; j <= MasterVariables.MAXSYSTEM; j++) {
			if (j > currentLevel || j == stateNumber || currentLevel == 2)
				...
				MasterVariables.state_profiles_writer.writeToFile("," + calcSimilarity(joiningProfiles[currentDeme], joiningProfiles[j - 1]));
		}
		...
		if (currentLevel == 2)
			...
			MasterVariables.state_profiles_writer.writeToFile("," + calcGenSimilarity(balanceProfiles) + "," + calcGenSimilarity(bandwagonProfiles)
					+ "," + calcGenSimilarity(joiningProfiles));
		}

		MasterVariables.state_profiles_writer.writeToFile("," + init_myCap_min + "," + init_myCap_med + "," + init_oppCap_min + "," + init_oppCap_med
		...
		if (currentLevel > 2) {
			MasterVariables.state_profiles_writer.writeToFile("," + balance_myCap_min + "," + balance_myCap_med + "," + balance_oppCap_min + ","
	...
	private static void printProfileWorlds() {
		if (currentGen % 50 != 0 || !(currentLevel == MasterVariables.MAXSYSTEM && currentDeme == currentLevel - 1))
			return;
		...
		MasterVariables.profiles_count_writer.writeToFile( "simulation" + "," + "generation" + "," + "genPerSimulation" + "," + "profile_no" + ","
		...
		for (int simulationIndex = 0; simulationIndex < currentSimulation; simulationIndex++) {
			...
			for (int i = 0; i < MasterVariables.uniqueWorldSizes; i++) {
				for (int k = 0; k < MasterVariables.PROFILES_PER_CAT; k++) {
					MasterVariables.profiles_count_writer.writeToFile(Integer.toString(simulationIndex + 1) + "," + currentGen + ","
					...
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
						if (j <= i + 1) {
							MasterVariables.profiles_count_writer.writeToFile("," + MasterVariables.totalProfileAttacks_PerState[simulationIndex][i][j][k]);
						} else
							MasterVariables.profiles_count_writer.writeToFile("," + ".");	
					}
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
				...
				if (i > 0)
					for (int k = 0; k < MasterVariables.PROFILES_PER_CAT; k++) {
						...
						for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
							if (j <= i + 1) {
								MasterVariables.profiles_count_writer.writeToFile("," + MasterVariables.totalProfileBalances_PerState[simulationIndex][i - 1][j][k]);
							} else
								MasterVariables.profiles_count_writer.writeToFile("," + ".");
						}
						for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
							if (j <= i + 1) {
								MasterVariables.profiles_count_writer.writeToFile("," + MasterVariables.totalProfileBandwagons_PerState[simulationIndex][i - 1][j][k]);
							} else
								MasterVariables.profiles_count_writer.writeToFile("," + ".");
						}
						for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
							if (j <= i + 1) {
								MasterVariables.profiles_count_writer.writeToFile("," + MasterVariables.totalProfileJoins_PerState[simulationIndex][i - 1][j][k]);
							} else
								MasterVariables.profiles_count_writer.writeToFile("," + ".");
						}
