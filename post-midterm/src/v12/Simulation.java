...
public class Simulation {
		...
		for (MasterVariables.currentGen = 1; MasterVariables.currentGen <= MasterVariables.MASTER_GENERATIONS; MasterVariables.currentGen++) {
			for (currentLevel = MasterVariables.MINSYSTEM; currentLevel <= MasterVariables.MAXSYSTEM; currentLevel++) {
				...
				for (currentDeme = 0; currentDeme < currentLevel; currentDeme++) {
					...
					displayStatus();
					/*
					 * if (currentLevel == 2 && MasterVariables.currentGen >
					 * 200) { printStatistics(); continue; }
					 */
					...
					for (int i = 0; i < MasterVariables.GENERATIONS_PER_DEME; i++) {
						...
						// XXX
						// testedStrategies = addBestStrategy(currentDeme, 0,
						// testedStrategies);
						...
						demes[currentLevel - 2][currentDeme].strategies = updateDeme(
								demes[currentLevel - 2][currentDeme].strategies, evolvedLevelStrategies);
	...
	private LevelStrategy[] updateDeme(LevelStrategy[] originalLevelStrategies, LevelStrategy[] evolvedLevelStrategies) {
		...
		for (int i = 0; i < MasterVariables.TOTAL_POPSIZE_PER_DEME; i++) {
			...
			// XXX
			// if (originalLevelStrategies[nIndex].fitness <
			// evolvedLevelStrategies[pIndex].fitness)
	...
	private int findBestStrategy(Strategy[] testedStrategies, int defaultBestIndex) {
		...
		// System.out.println("Fitness 0 is " +
		// testedStrategies[defaultBestIndex].fitness);
		...
		if (false && pbf > currentBestFit) {
			System.out
					.println("ERORRR !!! Simulation class .. previous best fitness is larger than current best fitness");
			System.exit(0);
		}
	...
	private static Strategy[] setupStrategies(LevelStrategy[] levelStrategies) {
		...
		for (int i = 0; i < MasterVariables.TOTAL_POPSIZE_PER_DEME; i++) {
			...
			testedStrategies[i].setInitStrategy(currentLevel, levelStrategies[i].init_strategy.clone());
			if (currentLevel > 2) {
				testedStrategies[i].setBalanceStrategy(currentLevel, levelStrategies[i].balance_strategy.clone());
				testedStrategies[i].setJoinStrategy(currentLevel, levelStrategies[i].join_strategy.clone());
