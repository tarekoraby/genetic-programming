...
public class Simulation {
			...
			for (currentLevel = MasterVariables.MINSYSTEM; currentLevel <= MasterVariables.MAXSYSTEM; currentLevel++) {
				...
				for (currentDeme = 0; currentDeme < currentLevel; currentDeme++) {
					...
					for (int i = 0; i < MasterVariables.GENERATIONS_PER_DEME; i++) {				
						...
						// bestStrategies[currentLevel -
						// 2][currentDeme].getLevelStrategy(currentLevel).deepCopy();
						calculateFitness();
						saveBestStrategy();
						evolve();
					}
	...
	private static void evolve() {
		...
		System.out.println( tempLevelStrategies.length + " " + evolvedStrategies.length);
		setupStrategies();
		calculateFitness();
		int nIndex;
	...
	private static void saveBestStrategy() {
		...
		if (pbf>testedStrategies[currentBestIndex].fitness){
			System.out.println("ERRRRRRRRROR!!!");
			System.exit(0);
		}
		pbf = testedStrategies[currentBestIndex].fitness;
		testedStrategies[0].fitness = -99;
		demes[currentLevel - 2][currentDeme].strategies[0].fitness = -99;
		
		/*bestStrategies[currentLevel - 2][currentDeme] = testedStrategies[currentBestIndex].deepCopy();
		bestStrategies[currentLevel - 2][currentDeme].bestStrategyFitness = testedStrategies[currentBestIndex].fitness;
		for (int i = 0; i < currentLevel; i++) {
			testedStrategies[i].fitness = -99;
