...
public class draft {
	...
	public static void main(String[] args) {
		...
		//worldSizes(2);
		//charTOInt();
		testWorldSizesCreation(2, 5);
	}
	
	private static void testWorldSizesCreation(int worldSize, int step) {
		ArrayList<double[]> obj = new ArrayList<double[]>();
		double[] temp = new double[worldSize];
		double nextBig;
		for (int max=100; max>nextBig; max-=step){
			temp[0]=max;
			
		}

		obj.trimToSize();
		double[][] result = (double[][]) obj.toArray();
