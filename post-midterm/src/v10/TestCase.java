...
public class TestCase {
	...
	private boolean findInitators() {
		for (int i = 0; i < initialSystemSize; i++) {
			if (surviving[i]) {
				checkRandNumCounter();
				if (i != testedStrategyIndex && randNums[randNumCounter++] < MasterVariables.RANDOMPLAYER_PROB) {
					if (randNums[randNumCounter++] *100  < randNums[randNumCounter++]) {
	...
	private void normalizeCapabilities() {
		...
		capabilities[lastSuvivIndex] = 1000 - sum;
		if (capabilities[lastSuvivIndex] == 0){
			capabilities[lastSuvivIndex] = 1;
			normalizeCapabilities();
		}

		/*for (int i = 0; i <= lastSuvivIndex; i++)
			if (surviving[i] & capabilities[i]==0){
				System.out.println(total+ " " + currentSystemSize + " " +i);
				System.exit(0);
			}*/
	}
