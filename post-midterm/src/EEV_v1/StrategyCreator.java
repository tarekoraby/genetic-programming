...
public class StrategyCreator {
	...
	Strategy[] populationCreator(int currentLevel, boolean isInit, int popSize){
		...
		if (isInit) {
			for (int counter = 0; counter < newStrategies.length; counter++) {
				...
				char[][] initStrategy = create_random_init_indiv(MasterVariables.DEPTH,
						MasterVariables.SIMULATECONSTRUCTIVISM, currentLevel);
				newStrategies[counter].setInitStrategy(currentLevel, initStrategy);
			}
		} else {
			for (int counter = 0; counter < newStrategies.length; counter++) {
				newStrategies[counter] = new Strategy(currentLevel, isInit);
				char[][] joinStrategy = create_random_join_indiv(MasterVariables.DEPTH,
						MasterVariables.SIMULATECONSTRUCTIVISM);
				newStrategies[counter].setJoinStrategy(currentLevel, joinStrategy);
	...
	Strategy create_random_strategy(int currentLevel, boolean isInit) {
		Strategy newStrategy = new Strategy(currentLevel, isInit);
		if (isInit) {
			char[][] initStrategy = create_random_init_indiv(MasterVariables.DEPTH,
					MasterVariables.SIMULATECONSTRUCTIVISM, currentLevel);
			newStrategy.setInitStrategy(currentLevel, initStrategy);
		} else {
			char[][] joinStrategy = create_random_join_indiv(MasterVariables.DEPTH,
					MasterVariables.SIMULATECONSTRUCTIVISM);
			newStrategy.setJoinStrategy(currentLevel, joinStrategy);
		}
		return newStrategy;
	}
	...
	char[][] create_random_join_indiv(int depth, boolean type2) {
		if (depth == 0)
			return (null);
		char[][] indiv = new char[2][];
		boolean[] takenIndex = new boolean[3];
		...
		for (int i = 0; i < indiv.length; i++) {
			...
			referentStateIndex[0] = (char) rd.nextInt(3);
