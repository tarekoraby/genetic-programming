...
public class draft3 {
	...
	private static void versionTwo() {
		...
		int MINSYSTEM = 8, MAXSYSTEM = 8;
		int step = 4;
		boolean onlyGreater = true;
		boolean uniqueCombinations = true;
		...
		for (int i = 0; i < uniqueWorldSizes; i++) {
			...
			cap_distributions[i] = caculateCapDistribution(onlyGreater, firstPlayerAmount, step, TOTAL_CAP_POOL, totalInitialPlayers);
		}
			...
			for (int i = 0; i < cap_distributions.length; i++) {
				for (int j = 0; j < cap_distributions[i].length; j++) {
					...
					if (uniqueCombinations)
						x = 2;
					for (int k = x; k < cap_distributions[i][j].length - 1; k++) {
		...
		for (int i = 0; i < cap_distributions.length; i++) {
			...
			for (int j = 0; j < cap_distributions[i].length; j++) {
				...
				for (int k = 0; k < cap_distributions[i][j].length; k++) {
					System.out.print((double) cap_distributions[i][j][k] * 100 / TOTAL_CAP_POOL + "\t");
				}
				System.out.println();
			}
			System.out.println("Total number of ways is : " + (cap_distributions[i].length - numOfIllegals[i]));
	...
	private static int[][] caculateCapDistribution(boolean onlyGreater, int firstPlayerAmount, int step, int total, int totalInitialPlayers) {
		int[][] capDistribution = new int[1000000][totalInitialPlayers];
		counter = 0;

		int[] array = new int[totalInitialPlayers];
		Split(onlyGreater, firstPlayerAmount, step, total, totalInitialPlayers, 0, array, capDistribution);
	...
	private static void Split(boolean onlyGreater, int firstPlayerAmount, int step, int total, int totalPlayers, int playerIndex,
		...
		for (int i = firstPlayerAmount; i <= total - totalPlayers + 1; i += step) {
			...
			remainder = total - i;
			Split(onlyGreater, step, step, remainder, totalPlayers - 1, playerIndex + 1, receivedArray, capDistribution);
		}
	}
}
