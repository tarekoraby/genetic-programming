...
import EEV_v4.SystemtoDyadPredictor;
	...
	private static void readProfilesCount() {
		...
		try {
			...
			for (int i = 0; i < numOfObs; i++) {
				...
				profiles[i].attackOrder = Integer.parseInt(tokens.nextToken().trim());
				...
				profiles[i].resources = new double[profiles[i].world_size];
				for (int k = 0; k < profiles[i].world_size; k++) {
					profiles[i].resources[k] = Double.parseDouble(tokens.nextToken().trim());
				}
			}
