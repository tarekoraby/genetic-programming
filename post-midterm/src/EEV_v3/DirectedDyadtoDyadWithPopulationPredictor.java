...
public class DirectedDyadtoDyadWithPopulationPredictor {
	static int[] years;
	static RealResNCode[][] resNCodes;
	static Profile[] profiles;
	static WriteFile predicted_writer;
	static String inputFileNameString = "C:\\Users\\taor9299\\Box Sync\\Java workspace\\post-midterm\\profilesV2.txt";
	static String realCapFileNameString = "C:\\Users\\taor9299\\Box Sync\\Java workspace\\post-midterm\\dyadic_inputs_predictor_with_pop.txt";
	static String outputFileNameString = "C:\\Users\\taor9299\\Box Sync\\Java workspace\\post-midterm\\output_predictor_dyadic_predictions_with_pop.txt";
	...
	private static void readProfilesCount() {
		...
		try {
			...
			for (int i = 0; i < numOfObs; i++) {
				...
				tempProfiles[i].populations = new double[tempProfiles[i].world_size];
				for (int k = 0; k < tempProfiles[i].world_size; k++) {
					tempProfiles[i].resources[k] = Double.parseDouble(tokens.nextToken().trim());
				}
				for (int k = 0; k < tempProfiles[i].world_size; k++) {
					tempProfiles[i].populations[k] = Double.parseDouble(tokens.nextToken().trim());
				}
		...
		try {
			...
			for (int i = 0; i < numOfObs; i++) {
				...
				for (int k = 0; k < 2; k++) {
					...
					resSum += resNCodes[i][k].resources;
				}			
				
				for (int k = 0; k < 2; k++) {
					resNCodes[i][k].resources = resNCodes[i][k].resources * 100 / resSum;
				}
				
				double popSum = 0;
				for (int k = 0; k < 2; k++) {
					resNCodes[i][k].population =  Double.parseDouble(tokens.nextToken().trim());
					popSum += resNCodes[i][k].population;
				}			
				
				for (int k = 0; k < 2; k++) {
					resNCodes[i][k].population = resNCodes[i][k].population * 100 / popSum;
				}
			}
	...
	private static double[] match(RealResNCode[] realResNCodes, int actorIndex, int targetIndex, Profile[] profiles) {
		...
		double[] populations = new double[realResNCodes.length];
		populations[0] = realResNCodes[actorIndex].population;
		counter = 1;
		for (int i = 0; i < realResNCodes.length; i++) {
			if (i == actorIndex)
				continue;
			if (i==targetIndex)
				targetIndex= counter;
			populations[counter++] = realResNCodes[i].population;
		}
		
		int mostSimilarProfileIndex = -1;
		double similarity, highestSimilarity = -1 * Double.MAX_VALUE;
		for (int i = 0; i < profiles.length; i++) {
			similarity = calcSimilarity(worldSize, resources, populations, targetIndex, profiles[i]);
	...
	private static double calcSimilarity(int worldSize, double[] resources, double[] populations, int targetIndex,
			Profile profile) {
		...
		for (int i = 0; i < resources.length; i++) {
			...
			similarity = similarity - Math.abs(populations[i] - profile.populations[i]);
		}
