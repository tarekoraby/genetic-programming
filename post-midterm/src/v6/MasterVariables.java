...
public final class MasterVariables {
	...
	static final int MASTER_GENERATIONS = 30000, POPSIZE_PER_PROCESSOR = 12500, MINRANDOM = 0, MAXRANDOM = 1,
	...
	static final int RANDOMNUMBERS = 50, CAPMED = RANDOMNUMBERS, CAPSTD = CAPMED + 1, CAPMIN = CAPSTD + 1,
			CAPMAX = CAPMIN + 1, MYCAP = CAPMAX + 1, OPPCAP = MYCAP + 1, SIDEACAP = OPPCAP + 1,
			LEFTCAPSUM = SIDEACAP + 1, MYENMITY = LEFTCAPSUM + 1, OPPENMITY = MYENMITY + 1, ADD = OPPENMITY + 1,
	...
	static final double RANDOMPLAYER_PROB = 1, ENHANCMENT_MARGIN = 0, PMUT_PER_NODE = 0, MOD_CROSSOVER_PROB = 0.2,
			REPLICATION_PROB = 0, CROSSOVER_PROB = 0.2, SUBTREE_MUT_PROB = 0.1, PMUT_PROB = 0.1,
			ABIOGENSIS_PROB = 0.4, GAUSS_PROB_FACTOR = 0;
	static final int INTERACTIONROUNDS = 10;
	static final boolean SIMULATECONSTRUCTIVISM = true;
	static final double[] randNum = new double[RANDOMNUMBERS];;
	static final Random rd = new Random();
	static final int[] TESTCASES = { 600, 2100};
	...
	private void initializeFitnessWorlds() {
		...
		for (int k = 0; k < uniqueWorldSizes; k++) {
			...
			for (int test = 0; test < TESTCASES[k]; test++) {
				...
				masterTestCasesRandNums[k][test] = new double[(int) (Math.pow(testedSize, testedSize) * testedSize * INTERACTIONROUNDS)];
				masterTestCasesRandGauss[k][test] = new double[(int) (Math.pow(testedSize, testedSize) * testedSize * INTERACTIONROUNDS)];
