...
public class StatisticsPrinter implements Callable<Void> {
	...
	static double total_inits, total_balances, total_bandwagons, total_joines, init_myCap_min, init_myCap_med,
			...
			init_sideACap_min, init_sideACap_med, init_leftCapSum_min, init_leftCapSum_med, init_myEnmity_min,
			...
			balance_capMax_min, balance_capMax_med, balance_sideACap_min, balance_sideACap_med,
			...
			bandwagon_sideACap_min, bandwagon_sideACap_med, bandwagon_leftCapSum_min, bandwagon_leftCapSum_med,
			bandwagon_myEnmity_min, bandwagon_myEnmity_med, bandwagon_oppEnmity_min, bandwagon_oppEnmity_med,
			join_myCap_min, join_myCap_med, join_oppCap_min, join_oppCap_med, join_capRatio_min,
			join_capRatio_med, join_capMed_min, join_capMed_med, join_capStd_min, join_capStd_med,
			join_capMin_min, join_capMin_med, join_capMax_min, join_capMax_med, join_sideACap_min,
			join_sideACap_med, join_leftCapSum_min, join_leftCapSum_med, join_myEnmity_min,
	...
	private static void populateProfiles() {
		...
		for (int i = 0; i < PROFILES_PER_CAT; i++) {
			attackProfiles[currentDeme][i] = profilingWorlds_Attacks[currentLevel - 2][i].willItAttack(
					bestLevelStrategy.init_strategy);
		...
		if (currentLevel > 2)
			for (int i = 0; i < PROFILES_PER_CAT; i++) {
				joiningProfiles[currentDeme][i] = profilingWorlds_Joins[currentLevel - 3][i].willItAttack(
						bestLevelStrategy.join_strategy);
				if (joiningProfiles[currentDeme][i]){
					MasterVariables.totalProfileJoins[currentLevel - 3][i]++;
					boolean output = profilingWorlds_Joins[currentLevel - 3][i].willItAttack(
							bestLevelStrategy.balance_strategy);
					if ((output && profilingWorlds_Joins[currentLevel - 3][i].smallerSideA)
							|| (!output && !profilingWorlds_Joins[currentLevel - 3][i].smallerSideA)) {
						balanceProfiles[currentDeme][i] = true;
						...
						bandwagonProfiles[currentDeme][i] = true;
	...
	private static void calculateProfileVariables() {
		...
		if (totalAttacks > 0) {
			...
			double[] init_sideACap = new double[totalAttacks];
			...
			for (int l = 0; l < PROFILES_PER_CAT; l++) {
				if (attackProfiles[currentDeme][l] == true) {
					...
					init_sideACap[totalAttacks - 1] = profilingWorlds_Attacks[indexWorldSizeAttacks][l].sideAcap;
			...
			init_sideACap_min = init_sideACap[0];
			...
			if (!even) {
				...
				init_sideACap_med = init_sideACap[(int) total_inits / 2];
				...
				init_sideACap_med = (init_sideACap[(int) total_inits / 2] + init_sideACap[(int) (total_inits / 2) - 1]) / 2;
		...
		if (currentLevel > 2) {
			...
			if (total_balances > 0) {
				...
				double[] balance_sideACap = new double[totalBalances];
				...
				for (int l = 0; l < PROFILES_PER_CAT; l++) {
					if (balanceProfiles[currentDeme][l] == true) {
						...
						balance_sideACap[totalBalances - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].sideAcap;
				...
				balance_sideACap_min = balance_sideACap[0];
				...
				if (!even) {
					...
					balance_sideACap_med = balance_sideACap[(int) total_balances / 2];
					...
					balance_sideACap_med = (balance_sideACap[(int) total_balances / 2] + balance_sideACap[(int) (total_balances / 2) - 1]) / 2;
			...
			if (total_bandwagons > 0) {
				...
				double[] bandwagon_sideACap = new double[totalBandwagons];
				...
				for (int l = 0; l < PROFILES_PER_CAT; l++) {
					if (bandwagonProfiles[currentDeme][l] == true) {
						...
						bandwagon_sideACap[totalBandwagons - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].sideAcap;
				...
				Arrays.sort(bandwagon_sideACap);
				...
				bandwagon_sideACap_min = bandwagon_sideACap[0];
				...
				if (!even) {
					...
					bandwagon_sideACap_med = bandwagon_sideACap[(int) total_bandwagons / 2];
					...
					bandwagon_sideACap_med = (bandwagon_sideACap[(int) total_bandwagons / 2] + bandwagon_sideACap[(int) (total_bandwagons / 2) - 1]) / 2;
			...
			total_joines = (double) totaljoines;

			if (total_joines > 0) {
				double[] join_myCap = new double[totaljoines];
				double[] join_oppCap = new double[totaljoines];
				double[] join_capRatio = new double[totaljoines];
				double[] join_capMed = new double[totaljoines];
				double[] join_capStd = new double[totaljoines];
				double[] join_capMin = new double[totaljoines];
				double[] join_capMax = new double[totaljoines];
				double[] join_sideACap = new double[totaljoines];
				double[] join_leftCapSum = new double[totaljoines];
				double[] join_myEnmity = new double[totaljoines];
				double[] join_oppEnmity = new double[totaljoines];

				int indexWorldSizeJoins = currentLevel - 3;

				for (int l = 0; l < PROFILES_PER_CAT; l++) {
					if (joiningProfiles[currentDeme][l] == true) {
						join_myCap[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].myCap;
						join_oppCap[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].oppCap;
						join_capRatio[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].capRatio;
						join_capMed[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].capMed;
						join_capStd[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].capStd;
						join_capMin[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].capMin;
						join_capMax[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].capMax;
						join_sideACap[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].sideAcap;
						join_leftCapSum[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].leftCapSum;
						join_myEnmity[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].myEnmity;
						join_oppEnmity[totaljoines - 1] = profilingWorlds_Joins[indexWorldSizeJoins][l].oppEnmity;
						totaljoines--;
					}
				}
				...
				Arrays.sort(join_capStd);
				Arrays.sort(join_capMin);
				Arrays.sort(join_capMax);
				Arrays.sort(join_sideACap);
				...
				join_capStd_min = join_capStd[0];
				join_capMin_min = join_capMin[0];
				join_capMax_min = join_capMax[0];
				join_sideACap_min = join_sideACap[0];
				join_leftCapSum_min = join_leftCapSum[0];
				join_myEnmity_min = join_myEnmity[0];
				join_oppEnmity_min = join_oppEnmity[0];

				even = total_joines % 2 == 0 ? true : false;
				if (!even) {
					join_myCap_med = join_myCap[(int) total_joines / 2];
					join_oppCap_med = join_oppCap[(int) total_joines / 2];
					join_capRatio_med = join_capRatio[(int) total_joines / 2];
					join_capMed_med = join_capMed[(int) total_joines / 2];
					join_capStd_med = join_capStd[(int) total_joines / 2];
					join_capMin_med = join_capMin[(int) total_joines / 2];
					join_capMax_med = join_capMax[(int) total_joines / 2];
					join_sideACap_med = join_sideACap[(int) total_joines / 2];
					join_leftCapSum_med = join_leftCapSum[(int) total_joines / 2];
					join_myEnmity_med = join_myEnmity[(int) total_joines / 2];
					join_oppEnmity_med = join_oppEnmity[(int) total_joines / 2];
				} else {
					join_myCap_med = (join_myCap[(int) total_joines / 2] + join_myCap[(int) (total_joines / 2) - 1]) / 2;
					join_oppCap_med = (join_oppCap[(int) total_joines / 2] + join_oppCap[(int) (total_joines / 2) - 1]) / 2;
					join_capRatio_med = (join_capRatio[(int) total_joines / 2] + join_capRatio[(int) (total_joines / 2) - 1]) / 2;
					join_capMed_med = (join_capMed[(int) total_joines / 2] + join_capMed[(int) (total_joines / 2) - 1]) / 2;
					join_capStd_med = (join_capStd[(int) total_joines / 2] + join_capStd[(int) (total_joines / 2) - 1]) / 2;
					join_capMin_med = (join_capMin[(int) total_joines / 2] + join_capMin[(int) (total_joines / 2) - 1]) / 2;
					join_capMax_med = (join_capMax[(int) total_joines / 2] + join_capMax[(int) (total_joines / 2) - 1]) / 2;
					join_sideACap_med = (join_sideACap[(int) total_joines / 2] + join_sideACap[(int) (total_joines / 2) - 1]) / 2;
					join_leftCapSum_med = (join_leftCapSum[(int) total_joines / 2] + join_leftCapSum[(int) (total_joines / 2) - 1]) / 2;
					join_myEnmity_med = (join_myEnmity[(int) total_joines / 2] + join_myEnmity[(int) (total_joines / 2) - 1]) / 2;
					join_oppEnmity_med = (join_oppEnmity[(int) total_joines / 2] + join_oppEnmity[(int) (total_joines / 2) - 1]) / 2;
				}
			}
		}
	}
	...
	private static void printProfileVariablesToFile() {
		...
		if (currentLevel == 2)
			...
			outputString += (total_balances + "," + total_bandwagons + "," + total_joines);
		...
		outputString += ("," + init_myCap_min + "," + init_myCap_med + "," + init_oppCap_min + "," + init_oppCap_med
				...
				+ init_capMax_min + "," + init_capMax_med + "," + init_sideACap_min + "," + init_sideACap_med + ","
		...
		if (currentLevel > 2) {
			outputString += ("," + balance_myCap_min + "," + balance_myCap_med + "," + balance_oppCap_min + ","
					...
					+ balance_capMax_min + "," + balance_capMax_med + "," + balance_sideACap_min + ","
					+ balance_sideACap_med + "," + balance_leftCapSum_min + "," + balance_leftCapSum_med + ","
					...
					+ bandwagon_sideACap_min + "," + bandwagon_sideACap_med + "," + bandwagon_leftCapSum_min + ","
					...
					+ join_myCap_med + "," + join_oppCap_min + "," + join_oppCap_med + ","
					+ join_capRatio_min + "," + join_capRatio_med + "," + join_capMed_min + ","
					+ join_capMed_med + "," + join_capStd_min + "," + join_capStd_med + ","
					+ join_capMin_min + "," + join_capMin_med + "," + join_capMax_min + ","
					+ join_capMax_med + "," + join_sideACap_min + "," + join_sideACap_med + ","
					+ join_leftCapSum_min + "," + join_leftCapSum_med + "," + join_myEnmity_min + ","
					+ join_myEnmity_med + "," + join_oppEnmity_min + "," + join_oppEnmity_med);
		}
	...
	private static void printProfileWorlds() {
		if (currentGen % 250 != 0 || !(currentLevel == MasterVariables.MAXSYSTEM && currentDeme == currentLevel - 1))
			return;

		String outputString = "";
		try {
			MasterVariables.profiles_count_writer = new WriteFile("profiles_count.txt", false);
			outputString = "generation" + "," + "profile_no" + ","
					+ "simulate_Joiner" + "," + "world_size" + "," + "capMed" + "," + "capStd" + "," + "capMin" + ","
					+ "capMax" + "," + "myCap" + "," + "oppCap-sideBcap" + "," + "capRatio-sideAtoBratio" + ","
					+ "sideACap" + "," + "leftCapSum" + "," + "myEnmity" + "," + "oppEnmity" + ","
		...
		for (int i = 0; i < MasterVariables.uniqueWorldSizes; i++) {
			for (int k = 0; k < MasterVariables.PROFILES_PER_CAT; k++) {
				outputString += (currentGen + "," + (k + 1) + "," + "0" + "," + (i + MasterVariables.MINSYSTEM) + ","
						...
						+ MasterVariables.profilingWorlds_Attacks[i][k].sideAcap + ","
			...
			if (i > 0)
				for (int k = 0; k < MasterVariables.PROFILES_PER_CAT; k++) {
					outputString += (currentGen + "," + (k + 1) + "," + "1" + "," + (i + MasterVariables.MINSYSTEM)
							...
							+ MasterVariables.totalProfileJoins[i - 1][k] );
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
						outputString += ("," + ".");
					}
					for (int j = 0; j < MasterVariables.MAXSYSTEM; j++) {
