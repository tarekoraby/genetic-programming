...
public class SystemtoDyadPredictor_basedOnContig {
	static int[] years;
	...
	static String realCapFileNameString = "C:\\Users\\taor9299\\Box Sync\\Java workspace\\post-midterm\\systemic_inputs_predictor_v2.txt";
	...
	private static void readRealCapData() {
		...
		try {
			...
			for (int i = 0; i < numOfObs; i++) {
				...
				//System.out.println(numOfStates[i]);
	...
	private static void predictConflict() {
		 ...
		 */
		int year, numOfStates, ccode_tested_state;
		double  total_attack_pred;
		double[] results1, results2=null;
		for (int counter = 0; counter < years.length; counter++) {
			year = years[counter];
			numOfStates = SystemtoDyadPredictor_basedOnContig.numOfStates[counter];
			ccode_tested_state = resNCodes[counter][0].ccode;
			...
			for (int i = 0; i < numOfStates; i++)
				if (resNCodes[counter][i].ccode == ccode_tested_state) {
					firstState = i;
					break;
				}
				
				for (int secondState = 0; secondState < numOfStates; secondState++) {
