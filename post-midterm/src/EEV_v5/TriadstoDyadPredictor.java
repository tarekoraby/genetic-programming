...
public class TriadstoDyadPredictor {
	static int[] years, triadIDs;
	...
	static String realCapFileNameString = "C:\\Users\\taor9299\\Box Sync\\Java workspace\\post-midterm\\triadic_inputs_predictor.txt";
	static String outputFileNameString = "C:\\Users\\taor9299\\Box Sync\\Java workspace\\post-midterm\\output_predictor_dyadic_predictions_basedOnTriads.txt";
	...
	private static void readRealCapData() {
		...
		try {
			...
			for (int i = 0; i < numOfObs; i++) {
				...
				triadIDs[i] = Integer.parseInt(tokens.nextToken().trim());
	...
	private static void predictConflict() {
		 ...
		 */
		int year, numOfState, triadID;
		double  total_attack_pred;
		double[] results1, results2;
		for (int counter = 0; counter < years.length; counter++) {
			year = years[counter];
			triadID = triadIDs[counter];
			numOfState = numOfStates[counter];
			resNCodes[counter] = sortOnResources(resNCodes[counter]);
			
			if (numOfState != resNCodes[counter].length){
				Thread.dumpStack();
				System.exit(0);
			}
			
			for (int firstState = 0; firstState < numOfState - 1; firstState++) {
				for (int secondState = firstState + 1; secondState < numOfState; secondState++) {
					...
					printToFile(year, triadID, resNCodes[counter][firstState].ccode, resNCodes[counter][secondState].ccode, results1, results2);
				}
			}
		}
	...
	private static void printToFile(int year, int triadID, int i, int j, double[] results1, double[] results2) {
		System.out.println(year + " " + triadID + " " +  i  + " " + j + " " + results1[0]+ " " + results2[0]+ " " + results1[1]+ " " + results2[1]+ " " + results1[2]+ " " + results2[2]);
		predicted_writer.writeNewLineToFile(year + " " + triadID + " " + i + " " + j + " " + results1[0]+ " " + results1[1]+ " " + results1[2] + "");
		predicted_writer.writeNewLineToFile(year + " " + triadID + " " + j + " " + i + " " + results2[0]+ " " + results2[1]+ " " + results2[2] + "");
