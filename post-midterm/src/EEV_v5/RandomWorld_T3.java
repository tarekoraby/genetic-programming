...
public class RandomWorld_T3 {
	...
	public RandomWorld_T3(int attackOrder, double[] resDistr, double[] terrDistr, int numOfRounds) {
	...
	double simulateRound() {
		...
		for (int i = 0; i < randInitStrategies.length; i++) {
			...
			/*if (terrDistr[testedStateIndex] > terrDistr[attackOrder])
				randJoinStrategies[i] = 2;
			else if (terrDistr[testedStateIndex] < terrDistr[attackOrder])
				randJoinStrategies[i] = 1;
			else 
				randJoinStrategies[i] = 0;*/
			//randJoinStrategies[i] = 2;
		...
		if ( worldSize!=3 || attackOrder== testedStateIndex || numOfRounds >1) {
		...
		sideA[attackOrder] = true;
		
		for (int i = 0; i < randInitStrategies.length; i++) {
			if (i == testedStateIndex || i == attackOrder)
				continue;
			sideB[i]=true;
		}
		
/*boolean sideATaken= false;
		...
		if ((attackOrder == testedStateIndex && realSideBCap > realSideACap + (realSideACap * attackersNecessarySuperiority))
				|| (attackOrder != testedStateIndex && realSideBCap + (realSideBCap * attackersNecessarySuperiority) > realSideACap)) {
		...
		for (int i = 0; i < resDistr.length; i++) {
			if (sideA[i]) {
				...
				//double share =  potentialTerrSpoil ;
				//double share =  terrDistr[attackOrder] ;
				terrDistr[i] += share;
			} 
		}
		...
		/*if (attackOrder != testedStateIndex)
			System.out.println(attackOrder + " " + origTerritory + " " + terrDistr[testedStateIndex]);*/
	...
	private void findJoiners(int[] randJoinStrategies, boolean[] sideA, boolean[] sideB) {
		for (int i = 0; i < randJoinStrategies.length; i++) {
			...
			if (randJoinStrategies[i] == 1) {
				...
				sideB[i] = true;
			} else if (randJoinStrategies[i] == 0 || randJoinStrategies[i] >2) {
