...
public class GP_PilotStudy_v7 {
	...
	static final int MAX_LEN = 10000, POPSIZE = 1000, DEPTH = 5, GENERATIONS = 10000, TSIZE = 2, BESTRETAINED = 100,
			MOD_CROSSOVERS = BESTRETAINED, TESTCASES = 5000, PROFILES = 1000, MINSYSTEM = 2, MAXSYSTEM = 3, RANDOMNUMBERS = 50, BLOATSTART=1000;
	static double MINRANDOM = 0, MAXRANDOM = 1;
	public static final double PMUT_PER_NODE = 0.05, REPLICATION_PROB = 0.2, CROSSOVER_PROB = 0.25,
			SUBTREE_MUT_PROB = 0.25, PMUT_PROB = 0.1, ABIOGENSIS_PROB = 0.2;
	...
	static final int CAPCHANGE = 5;
	World_System_Shell_v7[] profilingWorlds;
	boolean[][] profile;
	boolean[][] profile10;
	...
	public static void main(String[] args) {
		...
		GP_PilotStudy_v7 gp = new GP_PilotStudy_v7();
		gp.evolve();
		
	}

	public GP_PilotStudy_v7() {
		...
		profilingWorlds = new World_System_Shell_v7[PROFILES];
		for (int i = 0; i < PROFILES / 2; i++)
			profilingWorlds[i] = new World_System_Shell_v7(false);
		if (MAXSYSTEM > 2)
			for (int i = PROFILES / 2; i < PROFILES; i++)
				profilingWorlds[i] = new World_System_Shell_v7(true);
	...
	private void sortDesc(char[][] strategy_init_2, char[][] strategy_init_3, char[][] strategy_init_4, char[][] strategy_join_3, double[] fitness) {
		...
		GP_PilotStudy_v7.strategy_init_2 = strategy_init_2;
		GP_PilotStudy_v7.strategy_init_3 = strategy_init_3;
		GP_PilotStudy_v7.strategy_init_4 = strategy_init_4;
		GP_PilotStudy_v7.strategy_join_3 = strategy_join_3;
		...
		while (testCases > 0) {
			...
			while (counter < POPSIZE) {
				...
				for (int i = 0; i < length; i++) {
					...
					teststrategy_init_3[i] = strategy_init_3[counter + i].clone();
					teststrategy_init_4[i] = strategy_init_4[counter + i].clone();
					teststrategy_join_3[i] = strategy_join_3[counter + i].clone();
				}

				WorldSystem_v7 world = new WorldSystem_v7(teststrategy_init_2, teststrategy_init_3, teststrategy_init_4, teststrategy_join_3, length);
