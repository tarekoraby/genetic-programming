...
public class Simulation_v20 extends GP_PilotStudy_v20 {

	public Simulation_v20() {
	...
	private static void printStats(char[][][] init_strategy, char[][][] join_strategy) {
		...
		if (MAXSYSTEM > 2){
			...
			for (int i = 0; i < length; i++) {
				for (int k = 0; k < uniqueWorldSizes - 1; k++) {
					for (int l = 0; l < PROFILES_PER_CAT; l++) {
						if (balanceProfiles[i][k][l] == true || bandwagonProfiles[i][k][l] == true) {
							totalJoins++;
							joins[k]++;
							indiv_joins[i][k]++;
		...
		if (MAXSYSTEM > 2) {
			for (int i = 0; i < uniqueWorldSizes - 1; i++) {
				...
				for (int m = 0; m < joins[i]; m++) {
					for (int j = 0; j < length; j++) {
						for (int l = 0; l < PROFILES_PER_CAT; l++) {
							if (balanceProfiles[j][i][l] == true) {
								joinCapabilities[i][m] = profilingWorlds_Joins[i][l].myCap;
								joinCapabilitiesRatios[i][m++] = (profilingWorlds_Joins[i][l].myCap + profilingWorlds_Joins[i][l].smallerSideCap)
										/ profilingWorlds_Joins[i][l].largerSideCap;
							} else if (bandwagonProfiles[j][i][l] == true) {
								joinCapabilities[i][m] = profilingWorlds_Joins[i][l].myCap;
								joinCapabilitiesRatios[i][m++] = (profilingWorlds_Joins[i][l].myCap + profilingWorlds_Joins[i][l].largerSideCap)
										/ profilingWorlds_Joins[i][l].smallerSideCap;
							}
						}
					}
				}
		...
		for (int i = 0; i < length; i++) {
			...
			for (int l = 0; l < PROFILES_PER_CAT; l++) {
				outputString += ("," + (attackProfiles[i][currentTestedSize - MINSYSTEM][l] == true ? 1 : 0));

			}

			if (currentTestedSize > 2) {
				for (int l = 0; l < PROFILES_PER_CAT; l++) {
					outputString += ("," + (balanceProfiles[i][currentTestedSize - MINSYSTEM - 1][l] == true ? 1 : 0));
				}
				for (int l = 0; l < PROFILES_PER_CAT; l++) {
					outputString += ("," + (bandwagonProfiles[i][currentTestedSize - MINSYSTEM - 1][l] == true ? 1 : 0));
				}
			}
	...
	private static void initiateThreads(int operationID) {
		...
		implement = new implementClass_v20[processors];

		for (int threadID = 0; threadID < processors; threadID++) {
			implement[threadID] = new implementClass_v20(threadID, randNum, POPSIZE_PER_PROCESSOR,
	...
	private static void calcSimilarity(char[][][] init_strategy, char[][][] join_strategy, int indivs) {
		...
		balanceProfiles = new boolean[indivs][uniqueWorldSizes - 1][PROFILES_PER_CAT];
		bandwagonProfiles = new boolean[indivs][uniqueWorldSizes - 1][PROFILES_PER_CAT];

		for (int i = 0; i < indivs; i++) {
			for (int k = 0; k < currentTestedSize - MINSYSTEM + 1; k++) {
				for (int l = 0; l < PROFILES_PER_CAT; l++) {
					attackProfiles[i][k][l] = profilingWorlds_Attacks[k][l].willItAttack(init_strategy[i][k], 0);
				}
			}
		}
		...
		if (currentTestedSize > 2)
			for (int i = 0; i < indivs; i++) {
				for (int k = 0; k < currentTestedSize - MINSYSTEM; k++) {
					for (int l = 0; l < PROFILES_PER_CAT; l++) {
						balanceProfiles[i][k][l] = profilingWorlds_Joins[k][l].willItAttack(join_strategy[i][k], 1);
						bandwagonProfiles[i][k][l] = profilingWorlds_Joins[k][l].willItAttack(join_strategy[i][k], 2);
					}
				}
			}
