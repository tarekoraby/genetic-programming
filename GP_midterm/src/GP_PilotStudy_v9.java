...
public class GP_PilotStudy_v9 {
	double[] fitness;
	static double[] randNum;
	static char[][] strategy_init_2, strategy_init_3, strategy_init_4, strategy_join_3;
	static Random rd = new Random();
	static final int MAX_LEN = 10000, ROUNDS = 14, DEPTH = 5, GENERATIONS = 10000, TSIZE = 2, BESTRETAINED = 4,
			MOD_CROSSOVERS = 40, TESTCASES = 3000, PROFILES = 1000, MINSYSTEM = 2, MAXSYSTEM = 2,
			POPSIZE = (int) Math.pow(MAXSYSTEM, ROUNDS), RANDOMNUMBERS = 50, BLOATSTART=1000;
	static double MINRANDOM = 0, MAXRANDOM = 1;
	public static final double PMUT_PER_NODE = 0.05, REPLICATION_PROB = 0, CROSSOVER_PROB = 0,
			SUBTREE_MUT_PROB = 0, PMUT_PROB = 0, ABIOGENSIS_PROB = 1;
	...
	static double avg_len, similarityWarInit_2_Avg, similarityWarInit_3_Avg, similarityWarInit_4_Avg, similarityWarJoinAvg, tenWI_2_Similarity, tenWI_3_Similarity, tenWJSimilarity;
	static double[][] targets;
	static final boolean BLOATFIGHT = false, PRINTINDIV = false, PRINTPROFILE = false, PROFILEPOWER=false;
	static final int CAPCHANGE = 3;
	World_System_Shell_v9[] profilingWorlds;
	boolean[][] profile;
	boolean[][] profile10;
	...
	public static void main(String[] args) {
		...
		GP_PilotStudy_v9 gp = new GP_PilotStudy_v9();
		gp.evolve();
		
	}

	public GP_PilotStudy_v9() {
		...
		profilingWorlds = new World_System_Shell_v9[PROFILES];
		for (int i = 0; i < PROFILES / 2; i++)
			profilingWorlds[i] = new World_System_Shell_v9(false);
		if (MAXSYSTEM > 2)
			for (int i = PROFILES / 2; i < PROFILES; i++)
				profilingWorlds[i] = new World_System_Shell_v9(true);
	...
	void evolve() { 
		...
		fitness = calculateFitness(strategy_init_2, strategy_init_3, strategy_init_4, strategy_join_3, TESTCASES);
		stats( fitness, strategy_init_2, strategy_init_3, strategy_init_4, strategy_join_3, 0 );
		...
		for (int gen = 1; gen <= GENERATIONS; gen++) {
			...
			if (sortedFit) {
				...
				System.arraycopy(strategy_init_3, 0, tempstrategy_init_3, 0, BESTRETAINED);
				System.arraycopy(strategy_init_4, 0, tempstrategy_init_4, 0, BESTRETAINED);
				System.arraycopy(strategy_join_3, 0, tempstrategy_join_3, 0, BESTRETAINED);
			} else {
			...
			for (indivs = BESTRETAINED; indivs < MOD_CROSSOVERS + BESTRETAINED ; indivs++) { 
				...
				tempstrategy_init_2[indivs] = strategy_init_2[parents[rd.nextInt(3)]];
				tempstrategy_init_3[indivs] = strategy_init_3[parents[rd.nextInt(3)]];
				tempstrategy_init_4[indivs] = strategy_init_4[parents[rd.nextInt(3)]];
				tempstrategy_join_3[indivs] = strategy_join_3[parents[rd.nextInt(3)]];
			}
			...
			for (indivs =  MOD_CROSSOVERS + BESTRETAINED; indivs < POPSIZE; indivs++) {
				...
				if (random < REPLICATION_PROB) {
					...
					parent1 = tournament(fitness, TSIZE);
					parent2 = tournament(fitness, TSIZE);
					newind = crossover(strategy_init_3[parent1], strategy_init_3[parent2]);
					...
					TC10++;
					parent = tournament(fitness, TSIZE);
					newind = pointMutation(strategy_init_3[parent], PMUT_PER_NODE);
					tempstrategy_init_3[indivs] = newind;
				} else {
			...
			for (indivs =  MOD_CROSSOVERS + BESTRETAINED; indivs < POPSIZE; indivs++) {
				...
				if (random < REPLICATION_PROB) {
					...
					parent1 = tournament(fitness, TSIZE);
					parent2 = tournament(fitness, TSIZE);
					newind = crossover(strategy_init_4[parent1], strategy_init_4[parent2]);
					...
					TC10++;
					parent = tournament(fitness, TSIZE);
					newind = pointMutation(strategy_init_4[parent], PMUT_PER_NODE);
					tempstrategy_init_4[indivs] = newind;
				} else {
			...
			for (indivs =  MOD_CROSSOVERS + BESTRETAINED; indivs < POPSIZE; indivs++) {
				...
				if (random < REPLICATION_PROB) {
					...
					parent1 = tournament(fitness, TSIZE);
					parent2 = tournament(fitness, TSIZE);
					newind = crossover(strategy_join_3[parent1], strategy_join_3[parent2]);
					...
					TC10++;
					parent = tournament(fitness, TSIZE);
					newind = pointMutation(strategy_join_3[parent], PMUT_PER_NODE);
					tempstrategy_join_3[indivs] = newind;
				} else {
			...
			fitness = calculateFitness(strategy_init_2, strategy_init_3, strategy_init_4, strategy_join_3, TESTCASES);
			stats(fitness, strategy_init_2, strategy_init_3, strategy_init_4, strategy_join_3, gen);
	...
	private void sortDesc(char[][] strategy_init_2, char[][] strategy_init_3, char[][] strategy_init_4, char[][] strategy_join_3, double[] fitness) {
		...
		for (int i = 0; i < POPSIZE; i++) {
			...
			while (strategy_init_2[i] == null && j < POPSIZE) {
				if (fitness[i] == tempFitness[j]) {
					strategy_init_2[i] = (char[]) tempstrategy_init_2[j].clone();
					strategy_init_3[i] = (char[]) tempstrategy_init_3[j].clone();
					strategy_init_4[i] = (char[]) tempstrategy_init_4[j].clone();
					strategy_join_3[i] = (char[]) tempstrategy_join_3[j].clone();
		...
		GP_PilotStudy_v9.strategy_init_2 = strategy_init_2;
		GP_PilotStudy_v9.strategy_init_3 = strategy_init_3;
		GP_PilotStudy_v9.strategy_init_4 = strategy_init_4;
		GP_PilotStudy_v9.strategy_join_3 = strategy_join_3;
		this.fitness = fitness;
		
		sortedFit=true;
	}

	private void calcSimilarity(char[][] strategy_init_2, char[][] strategy_init_3, char[][] strategy_init_4, char[][] strategy_join_3, int indivs) {
		...
		for (int i = 0; i < indivs; i++){
			for (int j = 0; j < PROFILES / 2; j++)
				profile[i][j] = profilingWorlds[j].willItAttack(strategy_init_2[i], strategy_init_3[i], strategy_init_4[i], strategy_join_3[i]);
			if (MAXSYSTEM > 2)
	...
	void stats(double[] fitness, char[][] strategy_init_2, char[][] strategy_init_3, char[][] strategy_init_4, char[][] strategy_join_3, int gen) {
		if(!sortedFit){
			System.out.println("Not Sorted!!!");
			System.exit(0);
		}
		...
		favgpop /= POPSIZE;
		avg_len = (double) (node_count_1 + node_count_2 + node_count_3 + node_count_4) / POPSIZE; 
		
		calcSimilarity(strategy_init_2, strategy_init_3, strategy_init_4, strategy_join_3, BESTRETAINED);

		System.out.println("Generation=" + gen + "\nAvg Fitness=" + (favgpop) + " Median Fitness="
				+ fitness[POPSIZE / 2] + " Best Fitness=" + fitness[0] + " Worst Fitness=" + fitness[POPSIZE - 1]
				+ "\nAvg Size=" + avg_len );
		
		System.out.println("Among the top " + BESTRETAINED + " i.e. the top " + ((double) BESTRETAINED / POPSIZE * 100)
		...
		int totalAttacks = 0, totalAttacks_2 = 0, totalAttacks_3 = 0, totalJoins = 0, m = 0, n = 0;
		double cap2 = 0, min_cap2 = 1E16, cap3 = 0, min_cap3 = 1E16, cap3_join = 0, min_cap3_join = 1E16;
		...
		System.out.println("percentage of profile attacks 2 is: " + (double) totalAttacks_2 * 100 /  m + "%");
		System.out.println("percentage of profile attacks 3 is: " + (double) totalAttacks_3 * 100 /  n  + "%");
		System.out.println("And avg percentage of profile joins is: " + (double) totalJoins / (BESTRETAINED * (PROFILES - ((int) (PROFILES / 2)))) * 100 + "%");
		System.out.println("\navg cap of attackers in\n2 world " + cap2 / totalAttacks_2 );
	...
	double[] calculateFitness(char[][] strategy_init_2, char[][] strategy_init_3, char[][] strategy_init_4, char[][] strategy_join_3, int testCases) {
		// This method changes the order of the population arrays
		int[] prevIndexes, nextIndexes, groupIndexes;
		char[][] tempstrategy_init_2, tempstrategy_init_3, tempstrategy_init_4, tempstrategy_join_3;
		char[][] test_strategy_init_2, test_strategy_init_3, test_strategy_init_4, test_strategy_join_3;
		fitness = new double[POPSIZE];
		double[] wins, gamesPlayed;
		double tempDouble;
		int length, index, temp;
		boolean[] taken, survived;
		
		// Implementing Fisher–Yates shuffle of population arrays
		...
		prevIndexes = new int[POPSIZE];
		for (int i = 0; i < POPSIZE; i++) {
			prevIndexes[i] = i;
		}
		groupIndexes = new int[MAXSYSTEM];
		tempstrategy_init_2 = new char[MAXSYSTEM][];
		tempstrategy_init_3 = new char[MAXSYSTEM][];
		tempstrategy_init_4 = new char[MAXSYSTEM][];
		tempstrategy_join_3 = new char[MAXSYSTEM][];

		for (int round = 1; round <= ROUNDS; round++) {
			nextIndexes = new int[(int) Math.pow(MAXSYSTEM, ROUNDS - round)];
			for (int i = 0; i < prevIndexes.length; i += MAXSYSTEM) {

				for (int k = 0; k < MAXSYSTEM; k++) {
					groupIndexes[k] = prevIndexes[i + k];
					tempstrategy_init_2[k] = (char[]) strategy_init_2[prevIndexes[i + k]].clone();
					tempstrategy_init_3[k] = (char[]) strategy_init_3[prevIndexes[i + k]].clone();
					tempstrategy_init_4[k] = (char[]) strategy_init_4[prevIndexes[i + k]].clone();
					tempstrategy_join_3[k] = (char[]) strategy_join_3[prevIndexes[i + k]].clone();
				}

				wins = new double[MAXSYSTEM];
				gamesPlayed = new double[MAXSYSTEM];

				for (int m = 0; m < testCases; m++) {
					length = MINSYSTEM + rd.nextInt(MAXSYSTEM - MINSYSTEM + 1);
					if (length < MAXSYSTEM) {
						for (int n = MAXSYSTEM - 1; n > 0; n--) {
							index = rd.nextInt(n + 1);

							temp = groupIndexes[index];
							groupIndexes[index] = groupIndexes[n];
							groupIndexes[n] = temp;

							tempDouble = wins[index];
							wins[index] = wins[n];
							wins[n] = tempDouble;
							
							tempDouble = gamesPlayed[index];
							gamesPlayed[index] = gamesPlayed[n];
							gamesPlayed[n] = tempDouble;

							tempIndiv_1 = tempstrategy_init_2[index];
							tempstrategy_init_2[index] = tempstrategy_init_2[n];
							tempstrategy_init_2[n] = tempIndiv_1;

							tempIndiv_2 = tempstrategy_init_3[index];
							tempstrategy_init_3[index] = tempstrategy_init_3[n];
							tempstrategy_init_3[n] = tempIndiv_2;

							tempIndiv_3 = tempstrategy_init_4[index];
							tempstrategy_init_4[index] = tempstrategy_init_4[n];
							tempstrategy_init_4[n] = tempIndiv_3;

							tempIndiv_4 = tempstrategy_join_3[index];
							tempstrategy_join_3[index] = tempstrategy_join_3[n];
							tempstrategy_join_3[n] = tempIndiv_4;
						}
					}
					
					test_strategy_init_2 = new char[length][];
					test_strategy_init_3 = new char[length][];
					test_strategy_init_4 = new char[length][];
					test_strategy_join_3 = new char[length][];
					System.arraycopy(tempstrategy_init_2, 0, test_strategy_init_2, 0, length);
					System.arraycopy(tempstrategy_init_3, 0, test_strategy_init_3, 0, length);
					System.arraycopy(tempstrategy_init_4, 0, test_strategy_init_4, 0, length);
					System.arraycopy(tempstrategy_join_3, 0, test_strategy_join_3, 0, length);
					
					WorldSystem_v9 world = new WorldSystem_v9(test_strategy_init_2, test_strategy_init_3, test_strategy_init_4, test_strategy_join_3, length);
				...
				index=-1;
				double fbest = -1.0e34;
				for (int x = 0; x < MAXSYSTEM; x++) {
					if (wins[x] / gamesPlayed[x] > fbest + fbest*0.1){
						fbest= wins[x] / gamesPlayed[x];
						index = x;
					}
				}

				fitness[groupIndexes[index]]++;
				nextIndexes[i / MAXSYSTEM] = groupIndexes[index];
			}
		...
		if (BLOATFIGHT && X>=BLOATSTART) // punish longer strategies to fight bloating
			for (int i = 0; i < POPSIZE; i++)
		...
		sortedFit=false;
		sortDesc(strategy_init_2, strategy_init_3, strategy_init_4, strategy_join_3, fitness);
	...
	int tournament(double[] fitness,  int tsize) {
		...
		if (fbest == 0)
			return (tournament(fitness, tsize));
		else
			return (best);
	}
