...
public class implementClass_v26 implements Runnable {
	double[] fitness;
	static double[] randNum;
	Random rd = new Random();
	int TESTCASES, threadID;

	static final GP_PilotStudy_v26 X = new GP_PilotStudy_v26();
	...
	public implementClass_v26(){
		
	}

	public implementClass_v26(int threadID, double[] randNum, int POPSIZE, double[] fitness, char[][][] init_strategy,
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			for (int i = 0; i < POPSIZE; i++) {		
				...
				world = new WorldSystem_v26(length, testedStrategyIndex, capabilities, probabilities, GaussProb, changeIndexes, capChangeRates, initialEnmity);
			...
			if (rd.nextDouble() < pmut) {
				...
				if (parentcopy[mutsite] < X.FSET_1_START) {
					...
					case GP_PilotStudy_v26.EQ:
						...
						break;
					case GP_PilotStudy_v26.AND:
						parentcopy[mutsite] = GP_PilotStudy_v26.OR;
						break;
					case GP_PilotStudy_v26.OR:
						parentcopy[mutsite] = GP_PilotStudy_v26.AND;
						break;
					}
			}
		}
