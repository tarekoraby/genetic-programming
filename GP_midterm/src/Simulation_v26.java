...
public class Simulation_v26 extends GP_PilotStudy_v26 {
	
	
			

	public Simulation_v26() {
		...
		for (currentGen = 1; currentGen <= MASTER_GENERATIONS; currentGen++) {
			...
			StatisticsPrinter_v26 StatisticsPrinter=new StatisticsPrinter_v26();
			if (MAXSYSTEM == 2)
	...
	private void place_FT_in_Temp(int currentTestedIndex) {
		for (int j = 0; j <= currentTestedIndex; j++)
			temp_init_strategy[0][0][j] = (char[]) FT_init_strategy[currentTestedIndex][currentDeme][j]
					.clone();
		if (currentTestedIndex > 0) {
			for (int j = 0; j < uniqueWorldSizes - 1; j++) {
				temp_join_strategy[0][0][j] = (char[]) FT_join_strategy[currentTestedIndex - 1][currentDeme][j]
						.clone();
			}
		}
	...
	private static void simulation_initalize() {
		...
		implementClass_v26 implement = new implementClass_v26();
			...
			while (init_strategy[i][currentTestedSize - MINSYSTEM] == null && j < TOTAL_POPSIZE) {
				if (fitness[i] == tempFitness[j]) {
					...
					for (int k = 1; k < currentTestedSize - 1; k++) {
						...
						join_strategy[i][k - 1] = (char[]) tempjoin_strategy[j][k - 1].clone();
					}
	...
	private static void initiateThreads(int operationID) {
		...
		implement = new implementClass_v26[processors];

		for (int threadID = 0; threadID < processors; threadID++) {
			implement[threadID] = new implementClass_v26(threadID, randNum, POPSIZE_PER_PROCESSOR,
