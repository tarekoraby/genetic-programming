...
public class GP_PilotStudy_v15 {
	...
	static int GENERATIONS = 10000, POPSIZE_PER_PROCESSOR = 250, MINRANDOM = 0, MAXRANDOM = 1, PROFILES_PER_CAT = 150,
			MINSYSTEM = 2, MAXSYSTEM = 5, BESTRETAINED = 50, TESTCASES = 10000, MAX_LEN = 10000, DEPTH = 5, TSIZE = 10;
	...
	public static final double PMUT_PER_NODE = 0.05, MOD_CROSSOVER_PROB = 0.2, REPLICATION_PROB = 0.2,
			CROSSOVER_PROB = 0.1, SUBTREE_MUT_PROB = 0.1, PMUT_PROB = 0.1, ABIOGENSIS_PROB = 0.5;
	static final int CAPCHANGE = 1;
	static int currentGen, uniqueWorldSizes;
	static implementClass_v15[] implement;
	static Thread[] thread;
	private static int processors, TOTAL_POPSIZE;
	static double[] fitness, randNum;
	static char[][][] init_strategy, join_strategy;
	static World_System_Shell_v15[][] profilingWorlds_Attacks, profilingWorlds_Joins;
	...
	private static void initalize() {
		...
		profilingWorlds_Attacks = new World_System_Shell_v15[uniqueWorldSizes][PROFILES_PER_CAT];
		profilingWorlds_Joins = new World_System_Shell_v15[uniqueWorldSizes-1][PROFILES_PER_CAT];
		
		for (int i = 0; i < uniqueWorldSizes; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Attacks[i][k] = new World_System_Shell_v15(false, (i
						+ MINSYSTEM));
			}
		}
		...
		for (int i = 0; i < uniqueWorldSizes - 1; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Joins[i][k] = new World_System_Shell_v15(true, i
						+ MINSYSTEM + 1);
			}
		}
	...
	private static void initiateThreads(int operationID) {
		...
		implement = new implementClass_v15[processors];

		for (int threadID = 0; threadID < processors; threadID++) {
			implement[threadID] = new implementClass_v15(threadID, randNum, POPSIZE_PER_PROCESSOR,
