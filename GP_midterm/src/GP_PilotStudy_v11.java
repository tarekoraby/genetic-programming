...
public class GP_PilotStudy_v11 {
	double[] fitness;
	...
	static final int MAX_LEN = 10000, POPSIZE = 1000, DEPTH = 5,
			...
			MOD_CROSSOVERS = BESTRETAINED, TESTCASES = 3000, PROFILES = 3000,
			MINSYSTEM = 2, MAXSYSTEM = 3, RANDOMNUMBERS = 50, BLOATSTART = 100;
	static double MINRANDOM = 0, MAXRANDOM = 1;
	public static final double PMUT_PER_NODE = 0.05, REPLICATION_PROB = 0.15,
	...
	World_System_Shell_v11[] profilingWorlds;
	boolean[][] profile;
	boolean[][] profile10;
	...
	public static void main(String[] args) {
		...
		GP_PilotStudy_v11 gp = new GP_PilotStudy_v11();
		gp.evolve();
		
	}

	public GP_PilotStudy_v11() {
		...
		profilingWorlds = new World_System_Shell_v11[PROFILES];
		for (int i = 0; i < PROFILES / 2; i++)
			profilingWorlds[i] = new World_System_Shell_v11(false);
		if (MAXSYSTEM > 2)
			for (int i = PROFILES / 2; i < PROFILES; i++)
				profilingWorlds[i] = new World_System_Shell_v11(true);
		...
		for (gen=0; gen <= GENERATIONS; gen++) {
			...
			if (sortedFit) {
				for (indivs = 0 ; indivs < BESTRETAINED ; indivs++) { 
					tempstrategy_init_2[indivs] = (char[]) strategy_init_2[indivs].clone();
					tempstrategy_init_3[indivs] = (char[]) strategy_init_3[indivs].clone();
					tempstrategy_init_4[indivs] = (char[]) strategy_init_4[indivs].clone();
					tempstrategy_join_3[indivs] = (char[]) strategy_join_3[indivs].clone();
				}
			} else {
	...
	private void sortDesc(char[][] strategy_init_2, char[][] strategy_init_3, char[][] strategy_init_4, char[][] strategy_join_3, double[] fitness) {
		...
		GP_PilotStudy_v11.strategy_init_2 = strategy_init_2;
		GP_PilotStudy_v11.strategy_init_3 = strategy_init_3;
		GP_PilotStudy_v11.strategy_init_4 = strategy_init_4;
		GP_PilotStudy_v11.strategy_join_3 = strategy_join_3;
	...
	void stats(int gen) {
		...
		double favgpop = 0;
		int node_count_1 = 0, node_count_2 = 0, node_count_3 = 0, node_count_4 = 0;
		...
		int totalAttacks = 0, totalAttacks_2 = 0, totalAttacks_3 = 0,  totalAttacks_4 = 0,  totalJoins = 0, m = 0, n = 0, o = 0, p=0;
		double cap2 = 0, min_cap2 = 1E16, cap3 = 0, min_cap3 = 1E16, cap4= 0,  min_cap4 = 1E16, ratio4=0,  cap3_join = 0, min_cap3_join = 1E16, ratio3 = 0, min_ratio3=1E16,  min_ratio4=1E16;
		double[] cap3_min_List = new double[BESTRETAINED / 4];
		double[] cap4_min_List = new double[BESTRETAINED / 4];
			...
			for (int k = 0; k < PROFILES / 2; k++)
				if (profilingWorlds[k].currentStatesNum == 2) {
					...
					if (profile[i][k] == true) {
						...
						ratio3 += profilingWorlds[k].myCap
								/ profilingWorlds[k].oppCap;
						...
						if (profilingWorlds[k].myCap
								/ profilingWorlds[k].oppCap < min_ratio3)
							min_ratio3 = profilingWorlds[k].myCap
									/ profilingWorlds[k].oppCap;
					}
				} else if (profilingWorlds[k].currentStatesNum == 4) {
					p++;
					if (profile[i][k] == true) {
						...
						ratio4 += profilingWorlds[k].myCap
								/ profilingWorlds[k].oppCap;
						if (profilingWorlds[k].myCap < cap4_min_List[cap4_min_List.length - 1]) {
							cap4_min_List[cap4_min_List.length - 1] = profilingWorlds[k].myCap;
							Arrays.sort(cap4_min_List);
						}
						if (profilingWorlds[k].myCap < min_cap4)
							min_cap4 = profilingWorlds[k].myCap;
						if (profilingWorlds[k].myCap
								/ profilingWorlds[k].oppCap < min_ratio4)
							min_ratio4 = profilingWorlds[k].myCap
									/ profilingWorlds[k].oppCap;
					}
				}
			for (int k = PROFILES / 2; k < PROFILES; k++) {
		...
		double[] cap4_attackers_List = new double[totalAttacks_4];
		...
		for (int i = 0; i < BESTRETAINED; i++) {
			for (int k = 0; k < PROFILES / 2; k++)
				if (profilingWorlds[k].currentStatesNum == 2) {
					...
					if (profile[i][k] == true) {
						cap4_attackers_List[c++]= profilingWorlds[k].myCap;
					}
				}
			
			for (int k = PROFILES / 2; k < PROFILES; k++) {
				if (profile[i][k] == true) {
					cap3_joiners_List[d++]= profilingWorlds[k].myCap;
				}
			}
		...
		Arrays.sort(cap4_attackers_List);
		...
		if (totalAttacks_4>0)
		System.out.println("4 world " + cap4_attackers_List[totalAttacks_4/2]);
		...
		System.out.println("1st quartile of 4 world is: " + cap4_min_List[cap4_min_List.length-1]);
		System.out.println(ratio3/totalAttacks_3 + " " + min_ratio3);
		System.out.println(ratio4/totalAttacks_4 + " " + min_ratio4);
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			testedStrategyIndex = rd.nextInt(length);
			capabilities = new double[length];
			for (int i = 0; i < length; i++) {
				capabilities[i] = rd.nextDouble();

			}
			probabilities=new double[60];
			for (int i=0; i<60; i++)
				probabilities[i]=rd.nextDouble();

			for (int i = 0; i < POPSIZE; i++) {
				world = new WorldSystem_v11(length, testedStrategyIndex,
						capabilities, probabilities);
				boolean survived = world.simulate(strategy_init_2[i],
						strategy_init_3[i], strategy_init_4[i],
						strategy_join_3[i]);
