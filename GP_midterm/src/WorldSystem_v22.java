...
public class WorldSystem_v22 {
	...
	public WorldSystem_v22(int length, int[] testedStrategyIndex, double[] capabilities, double[] probabilities,
		...
		capChangeCount = 0;
		capChanges = GP_PilotStudy_v22.CAPCHANGE;
			...
			for (int i = 0; i < initialStatesNum; i++) {
				...
				if (surviving[i]) {
					if (strategy_init[i][capChangeCount][0] != null) {
						...
						strategy = strategy_init[i][capChangeCount][currentStatesNum - GP_PilotStudy_v22.MINSYSTEM];
						...
						for (int k = 0; k < initialStatesNum; k++) {
							...
							if (run() == 1) {
								enmity[i][k]++;
								isStable = false;
								if (enmity[i][k] == GP_PilotStudy_v22.ENMITYLEVELS - 1) {
