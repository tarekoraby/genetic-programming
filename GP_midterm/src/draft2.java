...
public class draft3 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < uniqueWorldSizes; i++) {
			...
			cap_distributions[i] = calculateDistributions(step, step, TOTAL_CAP_POOL, totalInitialPlayers);
	...
	private static int[][] calculateDistributions(int firstPlayerAmount, int step, int total, int totalInitialPlayers) {
		int[][] capDistribution=new int[1000][totalInitialPlayers];
		counter=0;
		
		Split(step, step, total, totalInitialPlayers, 0, capDistribution);
		
		
		int validDistributions=0;
		outer_loop: for (int i = 0; i < capDistribution.length; i++) {
			for (int k = 0; k < totalInitialPlayers; k++) {
				
				if (capDistribution[i][k] == 0)
					break outer_loop;
				System.out.print(capDistribution[i][k]);
			}
			System.out.println();
			validDistributions++;
		}
		...
		int[][] result=null;
		System.arraycopy(capDistribution, 0, result, 0, validDistributions);
