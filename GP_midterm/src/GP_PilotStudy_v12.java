...
public class GP_PilotStudy_v12 {
	double[] fitness;
	static double[] randNum;
	static char[][] strategy_init_2,		strategy_join_3;
	static Random rd = new Random();
	static final int MAX_LEN = 10000, POPSIZE = 1000, DEPTH = 5,
			GENERATIONS = 10000, TSIZE = 2, BESTRETAINED = 50,
			MOD_CROSSOVERS = BESTRETAINED, 
			TESTCASES = 3000, PROFILES = 2000, MINSYSTEM = 2, MAXSYSTEM = 3,
			RANDOMNUMBERS = 50, BLOATSTART = 1;
	static double MINRANDOM = 0, MAXRANDOM = 1;
	public static final double PMUT_PER_NODE = 0.05, REPLICATION_PROB = 0.1,
			CROSSOVER_PROB = 0.2, SUBTREE_MUT_PROB = 0.3, PMUT_PROB = 0.3,
			ABIOGENSIS_PROB = 0.1;
	...
	World_System_Shell_v12[] profilingWorlds;
	boolean[][] profile;
	boolean[][] profile10;
	...
	public static void main(String[] args) {
		...
		GP_PilotStudy_v12 gp = new GP_PilotStudy_v12();
		gp.evolve();
		
	}

	public GP_PilotStudy_v12() {
		...
		profilingWorlds = new World_System_Shell_v12[PROFILES];
		for (int i = 0; i < PROFILES / 2; i++)
			profilingWorlds[i] = new World_System_Shell_v12(false);
		if (MAXSYSTEM > 2)
			for (int i = PROFILES / 2; i < PROFILES; i++)
				profilingWorlds[i] = new World_System_Shell_v12(true);
	...
	void evolve() { 
		...
		int[] parents;
		char[][] tempstrategy_init_2,  tempstrategy_join_3;
	...
	private void sortDesc(char[][] strategy_init_2, char[][] strategy_join_3, double[] fitness) {
		...
		GP_PilotStudy_v12.strategy_init_2 = strategy_init_2;
		GP_PilotStudy_v12.strategy_join_3 = strategy_join_3;
		this.fitness = fitness;
		
		sortedFit=true;
	}

	private void calcSimilarity(char[][] strategy_init_2,  char[][] strategy_join_3, int indivs) {
		...
		for (int i = 0; i < indivs; i++){
			for (int j = 0; j < PROFILES / 2; j++)
				profile[i][j] = profilingWorlds[j].willItAttack(strategy_init_2[i],  strategy_join_3[i]);
			if (MAXSYSTEM > 2)
	...
	void stats(int gen) {
		...
		for (int i = 0; i < BESTRETAINED; i++) {
			...
			node_count_2 += traverse(strategy_join_3[i], 0);
		}
		favgpop /= BESTRETAINED;
		avg_len = (double) (node_count_1 + node_count_2) / POPSIZE; 
		
		calcSimilarity(strategy_init_2,  strategy_join_3, BESTRETAINED);
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			probabilities=new double[30];
			for (int i=0; i<30; i++)
				probabilities[i]=rd.nextDouble();

			for (int i = 0; i < POPSIZE; i++) {
				world = new WorldSystem_v12(length, testedStrategyIndex,
		...
		sortedFit=false;
		sortDesc(strategy_init_2, strategy_join_3, fitness);
