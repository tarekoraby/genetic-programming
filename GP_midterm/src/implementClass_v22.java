...
public class implementClass_v22 implements Runnable {
	double[] fitness;
	static double[] randNum;
	Random rd = new Random();
	int TESTCASES, threadID;

	static final GP_PilotStudy_v22 X = new GP_PilotStudy_v22();
	...
	public implementClass_v22(){
		
	}

	public implementClass_v22(int threadID, double[] randNum, int POPSIZE, double[] fitness, char[][][] init_strategy,
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			for (int m = 0; m <= X.CAPCHANGE; m++) {
				for (int i = 0; i < length; i++) {
					...
					if (t < X.currentTestedSize) {
						init_strategy[i][m][X.currentTestedSize - X.MINSYSTEM] = (char[]) X.FT_init_strategy[X.currentTestedSize
								- X.MINSYSTEM][t].clone();
						if (X.currentTestedSize > 2)
							join_strategy[i][m][X.currentTestedSize - X.MINSYSTEM - 1] = (char[]) X.FT_join_strategy[X.currentTestedSize
									- X.MINSYSTEM - 1][t].clone();
			...
			for (int i = 0; i < POPSIZE; i++) {		
				...
				world = new WorldSystem_v22(length, testedStrategyIndex, capabilities, probabilities, GaussProb, changeIndexes, capChangeRates, initialEnmity);
			...
			if (rd.nextDouble() < pmut) {
				...
				if (parentcopy[mutsite] < X.FSET_1_START) {
					...
					case GP_PilotStudy_v22.EQ:
						...
						break;
					case GP_PilotStudy_v22.AND:
						parentcopy[mutsite] = GP_PilotStudy_v22.OR;
						break;
					case GP_PilotStudy_v22.OR:
						parentcopy[mutsite] = GP_PilotStudy_v22.AND;
						break;
					}
			}
		}
