...
public class GP_PilotStudy_v20 {
	...
	static int DEMES = 3, MASTER_GENERATIONS = 10000, DEME_GENERATIONS = 1, POPSIZE_PER_PROCESSOR = 100, MINRANDOM = 0,
			...
			DEPTH = 5, TSIZE = 2, simulations = 1, stabilityNum = 10;
	...
	static World_System_Shell_v20[][] profilingWorlds_Attacks, profilingWorlds_Joins;
	...
	public static void main(String[] args) {
		...
		for (currentSimulation = 1; currentSimulation <= simulations; currentSimulation++) {
			Simulation_v20 simulation_v20 = new Simulation_v20();
		}
	}
	...
	private static void master_initalize() {
		...
		profilingWorlds_Attacks = new World_System_Shell_v20[uniqueWorldSizes][PROFILES_PER_CAT];
		profilingWorlds_Joins = new World_System_Shell_v20[uniqueWorldSizes - 1][PROFILES_PER_CAT];
		...
		for (int i = 0; i < uniqueWorldSizes; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Attacks[i][k] = new World_System_Shell_v20(false, (i + MINSYSTEM));
		...
		for (int i = 0; i < uniqueWorldSizes - 1; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Joins[i][k] = new World_System_Shell_v20(true, i + MINSYSTEM + 1);
		...
		for (int currentLevel = 0; currentLevel < TESTCASES.length; currentLevel++) {
			...
			for (int test = prevLevelTests; test < currentLevelTests; test++) {
				...
				int[] taken=new int[FT_init_strategy.length - 2] ;
				for (int i = 0; i < FT_init_strategy.length - 2; i++) 
					taken[i]=i;
				...
				for (int i = 0; i < length[test]; i++) {
					if (i==testedStrategyIndex[test][0])
						continue;
					int random = rd.nextInt(FT_init_strategy.length - 2);
					while (taken[random] < 0)
						random = rd.nextInt(FT_init_strategy.length - 2);
