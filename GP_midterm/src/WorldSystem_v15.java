...
public class WorldSystem_v15 {
	...
	public WorldSystem_v15(int length, int testedStrategyIndex, double[] capabilities, double[] probabilities) {
		...
		capChangeCount = GP_PilotStudy_v15.CAPCHANGE;
							...
							findJoiners: do {
								...
								for (int j = 0; j < initialStatesNum; j++) {
									...
									if (j != testedStrategyIndex) {
										...
										strategy = strategy_join[currentStatesNum - GP_PilotStudy_v15.MINSYSTEM - 1];
										int rand;
										if (probabilities[counter++] > probabilities[counter++]) {
											rand = 0;
										} else {
	...
	private double run() {
		...
		switch (primitive) {
		case GP_PilotStudy_v15.CAPMED:
			return (capMed);
		case GP_PilotStudy_v15.CAPAVG:
			return (capAvg);
		case GP_PilotStudy_v15.CAPSTD:
			return (capStd);
		case GP_PilotStudy_v15.CAPMIN:
			return (capMin);
		case GP_PilotStudy_v15.CAPMAX:
			return (capMax);
		case GP_PilotStudy_v15.MYCAP:
			return (myCap);
		case GP_PilotStudy_v15.OPPCAP:
			return (oppCap);
		case GP_PilotStudy_v15.MYSIDECAP:
			return (mySideCap);
		case GP_PilotStudy_v15.LEFTCAPSUM:
			return (leftCapSum);
		case GP_PilotStudy_v15.GT:
			if (run() > run())
				return (1);
			else
				return (0);
		case GP_PilotStudy_v15.LT:
			if (run() < run())
				return (1);
			else
				return (0);
		case GP_PilotStudy_v15.EQ:
			if (Math.abs(run() - run()) < 1E-10)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v15.AND:
			if (run() == 1 && run() == 1)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v15.OR:
			if (run() == 1 || run() == 1)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v15.ADD:
			return (run() + run());
		case GP_PilotStudy_v15.SUB:
			return (run() - run());
		case GP_PilotStudy_v15.MUL:
			return (run() * run());
		case GP_PilotStudy_v15.DIV: {
		...
		default:
			return (GP_PilotStudy_v15.randNum[primitive]);
		}
