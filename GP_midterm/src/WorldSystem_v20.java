...
public class WorldSystem_v20 {
	...
	public WorldSystem_v20(int length, int[] testedStrategyIndex, double[] capabilities, double[] probabilities,
		...
		capChangeCount = 0;
		capChanges = GP_PilotStudy_v20.CAPCHANGE;
			...
			for (int i = 0; i < initialStatesNum; i++) {
				...
				if (surviving[i]) {
					if (strategy_init[i][capChangeCount][0] != null) {
						...
						strategy = strategy_init[i][capChangeCount][currentStatesNum - GP_PilotStudy_v20.MINSYSTEM];
	...
	private double run() {
		...
		switch (primitive) {
		case GP_PilotStudy_v20.CAPMED:
			return (capMed);
		case GP_PilotStudy_v20.CAPAVG:
			return (capAvg);
		case GP_PilotStudy_v20.CAPSTD:
			return (capStd);
		case GP_PilotStudy_v20.CAPMIN:
			return (capMin);
		case GP_PilotStudy_v20.CAPMAX:
			return (capMax);
		case GP_PilotStudy_v20.MYCAP:
			return (myCap);
		case GP_PilotStudy_v20.OPPCAP:
			return (oppCap);
		case GP_PilotStudy_v20.MYSIDECAP:
			return (mySideCap);
		case GP_PilotStudy_v20.LEFTCAPSUM:
			return (leftCapSum);
		...
		default:
			return (GP_PilotStudy_v20.randNum[primitive]);
		}
