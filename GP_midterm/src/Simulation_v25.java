...
public class Simulation_v25 extends GP_PilotStudy_v25 {
	
	
			

	public Simulation_v25() {
		...
		for (currentGen = 1; currentGen <= MASTER_GENERATIONS; currentGen++) {
			...
			StatisticsPrinter_v25 StatisticsPrinter=new StatisticsPrinter_v25();
			if (MAXSYSTEM == 2)
	...
	private void setRandommness() {
		tempTestedStrategyIndexes = new int[TOTAL_TESTCASES][CAPCHANGE + 1][];
		...
		for (int currentLevel = 0; currentLevel < TESTCASES.length; currentLevel++) {
			...
			for (int test = prevLevelTests; test < currentLevelTests; test++) {
				...
				for (int k = 0; k <= CAPCHANGE; k++) {
					tempTestedStrategyIndexes[test][k] = (int[]) testStrategyIndexes[test][k].clone();
				}
	...
	private static void simulation_initalize() {
		...
		implementClass_v25 implement = new implementClass_v25();
	...
	private static void initiateThreads(int operationID) {
		...
		implement = new implementClass_v25[processors];

		for (int threadID = 0; threadID < processors; threadID++) {
			implement[threadID] = new implementClass_v25(threadID, randNum, POPSIZE_PER_PROCESSOR,
