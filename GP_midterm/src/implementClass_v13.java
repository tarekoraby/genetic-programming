...
public class implementClass_v13 implements Runnable {
	double[] fitness;
	static double[] randNum;
	Random rd = new Random();
	int TESTCASES, threadID;
	static final int MAX_LEN = 10000, DEPTH = 5, GENERATIONS = 10000,
			TSIZE = 4, BESTRETAINED = 50, MOD_CROSSOVERS = BESTRETAINED,
			PROFILES = 1000, MINSYSTEM = 2, MAXSYSTEM = 3,
			RANDOMNUMBERS = 50, BLOATSTART = 1;
	static double MINRANDOM = 0, MAXRANDOM = 1;
	public static final double PMUT_PER_NODE = 0.05, REPLICATION_PROB = 0.2,
			CROSSOVER_PROB = 0.2, SUBTREE_MUT_PROB = 0.2, PMUT_PROB = 0.1,
			ABIOGENSIS_PROB = 0.3;
	...
	public implementClass_v13(int threadID, double[] randNum, int POPSIZE, double[] fitness,
	...
	public void run() {
		...
		if (operationID == 1) {
			GP_PilotStudy_v13.temp_init_strategy[threadID]=create_random_pop(POPSIZE, DEPTH);
			GP_PilotStudy_v13.temp_join_strategy[threadID]=create_random_pop(POPSIZE, DEPTH);
		} else if (operationID == 2) {
			GP_PilotStudy_v13.tempFitness[threadID]=calculateFitness(TESTCASES);
	...
	char[][] create_random_pop(int popSize, int depth) {
	...
	void evolve() {
		...
		GP_PilotStudy_v13 x = new GP_PilotStudy_v13();
		...
		for (indivs = 0; indivs < MOD_CROSSOVERS; indivs++) {
			...
			tempinit_strategy[indivs] = init_strategy[parents[rd.nextInt(3)]];

			tempjoin_strategy[indivs] = join_strategy[parents[rd.nextInt(3)]];
		}
		...
		for (indivs = MOD_CROSSOVERS; indivs < POPSIZE; indivs++) {
			...
			if (random < REPLICATION_PROB) {
				...
				tempinit_strategy[indivs] = init_strategy[parent];
			} else if ((random - REPLICATION_PROB) < CROSSOVER_PROB) {
				TC8++;
				parent1 = tournament(fitness, TSIZE);
				parent2 = tournament(fitness, TSIZE);
				newind = crossover(init_strategy[parent1],
						init_strategy[parent2]);
				tempinit_strategy[indivs] = newind;
			} else if ((random - (CROSSOVER_PROB + REPLICATION_PROB)) < SUBTREE_MUT_PROB) {
				TC9++;
				parent = tournament(fitness, TSIZE);
				newind = subtreeMutation(init_strategy[parent]);
				tempinit_strategy[indivs] = newind;
			} else if ((random - (CROSSOVER_PROB + REPLICATION_PROB + SUBTREE_MUT_PROB)) < PMUT_PROB) {
				TC10++;
				parent = tournament(fitness, TSIZE);
				newind = pointMutation(init_strategy[parent], PMUT_PER_NODE);
				tempinit_strategy[indivs] = newind;
			} else {
				tempinit_strategy[indivs] = create_random_indiv(DEPTH);
			}
		}
		...
		for (indivs = MOD_CROSSOVERS; indivs < POPSIZE; indivs++) {
			...
			if (random < REPLICATION_PROB) {
				TC13++;
				parent = tournament(fitness, TSIZE);
				tempjoin_strategy[indivs] = join_strategy[parent];
			} else if ((random - REPLICATION_PROB) < CROSSOVER_PROB) {
				TC8++;
				parent1 = tournament(fitness, TSIZE);
				parent2 = tournament(fitness, TSIZE);
				newind = crossover(join_strategy[parent1],
						join_strategy[parent2]);
				tempjoin_strategy[indivs] = newind;
			} else if ((random - (CROSSOVER_PROB + REPLICATION_PROB)) < SUBTREE_MUT_PROB) {
				TC9++;
				parent = tournament(fitness, TSIZE);
				newind = subtreeMutation(join_strategy[parent]);
				tempjoin_strategy[indivs] = newind;
			} else if ((random - (CROSSOVER_PROB + REPLICATION_PROB + SUBTREE_MUT_PROB)) < PMUT_PROB) {
				TC10++;
				parent = tournament(fitness, TSIZE);
				newind = pointMutation(join_strategy[parent], PMUT_PER_NODE);
				tempjoin_strategy[indivs] = newind;
			} else {
				tempjoin_strategy[indivs] = create_random_indiv(DEPTH);
			}
		}

		GP_PilotStudy_v13.temp_init_strategy[threadID] = tempinit_strategy;
		GP_PilotStudy_v13.temp_join_strategy[threadID] = tempjoin_strategy;
	}
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			for (int i = 0; i < POPSIZE; i++) {
				world = new WorldSystem_v13(length, testedStrategyIndex,
						capabilities, probabilities);
				boolean survived = world.simulate(init_strategy[i],
											join_strategy[i]);
