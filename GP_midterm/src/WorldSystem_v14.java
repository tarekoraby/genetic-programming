...
public class WorldSystem_v14 {
	...
	public char[][] strategy_init, strategy_join;
	char[] strategy;
	boolean[] warringA, warringB, surviving;
	int capChangeCount, testedStrategyIndex, counter;

	public WorldSystem_v14(int length, int testedStrategyIndex, double[] capabilities, double[] probabilities) {
		...
		capChangeCount = implementClass_v14.CAPCHANGE;
	...
	public boolean simulate(char[][] strategy_init,  char[][] strategy_join) {
		...
		this.strategy_join = strategy_join;
		isStable = false;
		while (!isStable) {
			calculateStatesNo();
			...
			for (int i = 0; i < initialStatesNum; i++) {
				...
				if (surviving[i]) {
					if (i == testedStrategyIndex) {
						...
						mySideCap = myCap;
						strategy = strategy_init[currentStatesNum
								- implementClass_v14.MINSYSTEM];
						...
						if (probabilities[counter++] > probabilities[counter++]) {
							...
							if (currentStatesNum == 2) {
								targetIndex = testedStrategyIndex;
							} else if (probabilities[counter++] > probabilities[counter++]) {
								targetIndex = testedStrategyIndex;
							} else {
							...
							findJoiners: do {
								...
								for (int j = 0; j < initialStatesNum; j++) {
									...
									if (j != testedStrategyIndex) {
										...
										strategy = strategy_join[currentStatesNum - implementClass_v14.MINSYSTEM - 1];
										int rand;
										if (probabilities[counter++] > probabilities[counter++]) {
											rand = 0;
										} else {
	...
	private void excuteAttack() {
		...
		implementClass_v14.TC6++;
		implementClass_v14.TC12++;
	}
	...
	private double run() {
		...
		switch (primitive) {
		case implementClass_v14.CAPMED:
			return (capMed);
		case implementClass_v14.CAPMIN:
			return (capMin);
		case implementClass_v14.CAPMAX:
			return (capMax);
		case implementClass_v14.MYCAP:
			return (myCap);
		case implementClass_v14.OPPCAP:
			return (oppCap);
		case implementClass_v14.MYSIDECAP:
			return (mySideCap);
		case implementClass_v14.LEFTCAPSUM:
			return (leftCapSum);
		case implementClass_v14.GT:
			if (run() > run())
				return (1);
			else
				return (0);
		case implementClass_v14.LT:
			if (run() < run())
				return (1);
			else
				return (0);
		case implementClass_v14.EQ:
			if (Math.abs(run() - run()) < 1E-10)
				return (1);
			else
				return (0);
		case implementClass_v14.AND:
			if (run() == 1 && run() == 1)
				return (1);
			else
				return (0);
		case implementClass_v14.OR:
			if (run() == 1 || run() == 1)
				return (1);
			else
				return (0);
		case implementClass_v14.ADD:
			return (run() + run());
		case implementClass_v14.SUB:
			return (run() - run());
		case implementClass_v14.MUL:
			return (run() * run());
		case implementClass_v14.DIV: {
		...
		default:
			return (implementClass_v14.randNum[primitive]);
		}
