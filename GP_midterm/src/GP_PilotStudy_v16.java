...
public class GP_PilotStudy_v16 {
	public static final boolean PROFILEPOWER = false;
	static int GENERATIONS = 10000, POPSIZE_PER_PROCESSOR = 250, MINRANDOM = 0, MAXRANDOM = 1, PROFILES_PER_CAT = 150,
			MINSYSTEM = 2, MAXSYSTEM = 5, BESTRETAINED = 50, TESTCASES = 1000, MAX_LEN = 10000, DEPTH = 5, TSIZE = 10;
	static final int RANDOMNUMBERS = 50, 
			CAPMED = RANDOMNUMBERS,
			CAPAVG = RANDOMNUMBERS + 1,
			CAPSTD = RANDOMNUMBERS + 2,
			CAPMIN = CAPMED + 3,
			CAPMAX = CAPMED + 4,
			MYCAP = CAPMED + 5,
			OPPCAP = CAPMED + 6,
	...
	public static final double PMUT_PER_NODE = 0.05, MOD_CROSSOVER_PROB = 0.2, REPLICATION_PROB = 0.2,
			CROSSOVER_PROB = 0.2, SUBTREE_MUT_PROB = 0.2, PMUT_PROB = 0.2, ABIOGENSIS_PROB = 0.2;
	static final int CAPCHANGE = 1;
	static int currentGen, uniqueWorldSizes;
	static implementClass_v16[] implement;
	static Thread[] thread;
	private static int processors, TOTAL_POPSIZE;
	static double[] fitness, randNum;
	static char[][] init_strategy, join_strategy;
	static World_System_Shell_v16[][] profilingWorlds_Attacks, profilingWorlds_Joins;
	...
	private static void printStats() {
		...
		System.out.println("\navg percentage of profile joins is: "
				+ (double) totalJoins * 100 / (BESTRETAINED * uniqueWorldSizes * PROFILES_PER_CAT)  + "%");
		System.out.println("percentage of profile joins in a " );
		for (int i=0; i<uniqueWorldSizes-1; i++){
	...
	private static void initalize() {
		...
		profilingWorlds_Attacks = new World_System_Shell_v16[uniqueWorldSizes][PROFILES_PER_CAT];
		profilingWorlds_Joins = new World_System_Shell_v16[uniqueWorldSizes-1][PROFILES_PER_CAT];
		
		for (int i = 0; i < uniqueWorldSizes; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Attacks[i][k] = new World_System_Shell_v16(false, (i
						+ MINSYSTEM));
			}
		}
		...
		for (int i = 0; i < uniqueWorldSizes - 1; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Joins[i][k] = new World_System_Shell_v16(true, i
						+ MINSYSTEM + 1);
			}
		}
		...
		temp_init_strategy=new char[processors][POPSIZE_PER_PROCESSOR][];
		temp_join_strategy=new char[processors][POPSIZE_PER_PROCESSOR][];
		...
		for (int test = 0; test < TESTCASES; test++) {
			...
			probabilities[test] = new double[20*MAXSYSTEM];
			for (int i = 0; i < 20*MAXSYSTEM; i++)
	...
	private static void initiateThreads(int operationID) {
		...
		implement = new implementClass_v16[processors];

		for (int threadID = 0; threadID < processors; threadID++) {
			implement[threadID] = new implementClass_v16(threadID, randNum, POPSIZE_PER_PROCESSOR,
