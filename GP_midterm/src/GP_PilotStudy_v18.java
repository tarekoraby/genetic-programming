...
public class GP_PilotStudy_v18 {
	...
	static int DEMES = 3, MASTER_GENERATIONS = 10000, DEME_GENERATIONS = 1, POPSIZE_PER_PROCESSOR = 100, MINRANDOM = 0,
			MAXRANDOM = 1, PROFILES_PER_CAT = 500, MINSYSTEM = 2, MAXSYSTEM = 3, TESTCASES = 2000, MAX_LEN = 10000,
			DEPTH = 5, TSIZE = 10, simulations = 10, stabilityNum = 100;
	...
	public static final double RANDOMPLAYER_PROB = 0.05, ENHANCMENT_MARGIN = 0, PMUT_PER_NODE = 0.1,
			MOD_CROSSOVER_PROB = 0.25, REPLICATION_PROB = 0.25, CROSSOVER_PROB = 0, SUBTREE_MUT_PROB = 0, PMUT_PROB = 0,
			ABIOGENSIS_PROB = 0.5, GAUSS_PROB_FACTOR = 0;
	static final int CAPCHANGE = 5;
	static int currentGen, uniqueWorldSizes, currentSimulation, currentTestedSize;
	static implementClass_v18[] implement;
	static Thread[] thread;
	static int processors, TOTAL_POPSIZE;
	...
	static World_System_Shell_v18[][] profilingWorlds_Attacks, profilingWorlds_Joins;
	...
	public static void main(String[] args) {
		...
		for (currentSimulation = 1; currentSimulation <= simulations; currentSimulation++) {
			Simulation_v18 simulation_v18 = new Simulation_v18();
		}
	}
	...
	private static void master_initalize() {
		...
		profilingWorlds_Attacks = new World_System_Shell_v18[uniqueWorldSizes][PROFILES_PER_CAT];
		profilingWorlds_Joins = new World_System_Shell_v18[uniqueWorldSizes - 1][PROFILES_PER_CAT];
		...
		for (int i = 0; i < uniqueWorldSizes; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Attacks[i][k] = new World_System_Shell_v18(false, (i + MINSYSTEM));
		...
		for (int i = 0; i < uniqueWorldSizes - 1; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Joins[i][k] = new World_System_Shell_v18(true, i + MINSYSTEM + 1);
