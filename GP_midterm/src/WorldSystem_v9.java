...
public class WorldSystem_v9 {
	...
	public WorldSystem_v9(char[][] strategy_init_2, char[][] strategy_init_3, char[][] strategy_init_4, char[][] strategy_join_3, int length) {
		...
		GP_PilotStudy_v9.TC1++; 
		GP_PilotStudy_v9.TC11++; 
		GP_PilotStudy_v9.TC2 +=initialStatesNum;
		...
		GP_PilotStudy_v9.TC5 += capMed;
		
		
		capChangeCount = GP_PilotStudy_v9.CAPCHANGE;
	...
	private void excuteAttack() {
		...
		GP_PilotStudy_v9.TC6++;
		GP_PilotStudy_v9.TC12++;
	}
	...
	private double run() {
		...
		switch (primitive) {
		case GP_PilotStudy_v9.CAPMED:
			return (capMed);
		case GP_PilotStudy_v9.CAPMIN:
			return (capMin);	
		case GP_PilotStudy_v9.CAPMAX:
			return (capMax);
		case GP_PilotStudy_v9.MYCAP:
			return (myCap);
		case GP_PilotStudy_v9.OPPCAP:
			return (oppCap);
		case GP_PilotStudy_v9.MYSIDECAP:
			return (mySideCap);
		case GP_PilotStudy_v9.LEFTCAPSUM:
			return (leftCapSum);
		case GP_PilotStudy_v9.GT:
			if (run() > run())
				return (1);
			else
				return (0);
		case GP_PilotStudy_v9.LT:
			if (run() < run())
				return (1);
			else
				return (0);
		case GP_PilotStudy_v9.EQ:
			if (run() == run())
				return (1);
			else
				return (0);
		case GP_PilotStudy_v9.AND:
			if (run() == 1 && run() == 1)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v9.OR:
			if (run() == 1 || run() == 1)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v9.ADD:
			return (run() + run());
		case GP_PilotStudy_v9.SUB:
			return (run() - run());
		case GP_PilotStudy_v9.MUL:
			return (run() * run());
		case GP_PilotStudy_v9.DIV: {
		...
		default:
			return (GP_PilotStudy_v9.randNum[primitive]);
		}
