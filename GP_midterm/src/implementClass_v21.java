...
public class implementClass_v21 implements Runnable {
	double[] fitness;
	static double[] randNum;
	Random rd = new Random();
	int TESTCASES, threadID;

	static final GP_PilotStudy_v21 X = new GP_PilotStudy_v21();
	...
	public implementClass_v21(){
		
	}

	public implementClass_v21(int threadID, double[] randNum, int POPSIZE, double[] fitness, char[][][] init_strategy,
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			for (int i = 0; i < POPSIZE; i++) {		
				...
				world = new WorldSystem_v21(length, testedStrategyIndex, capabilities, probabilities, GaussProb, changeIndexes, capChangeRates);
			...
			if (rd.nextDouble() < pmut) {
				...
				if (parentcopy[mutsite] < X.FSET_1_START) {
					...
					case GP_PilotStudy_v21.EQ:
						...
						break;
					case GP_PilotStudy_v21.AND:
						parentcopy[mutsite] = GP_PilotStudy_v21.OR;
						break;
					case GP_PilotStudy_v21.OR:
						parentcopy[mutsite] = GP_PilotStudy_v21.AND;
						break;
					}
			}
		}
