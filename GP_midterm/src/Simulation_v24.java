...
public class Simulation_v24 extends GP_PilotStudy_v24 {
	...
	public void Start() {
		...
		for (currentGen = 1; currentGen <= MASTER_GENERATIONS; currentGen++) {
			...
			StatisticsPrinter_v24 StatisticsPrinter=new StatisticsPrinter_v24();
			if (MAXSYSTEM == 2)
	...
	private void setTestCases() {
		length = super.length[currentGen-1]; 
		testedStrategyIndex = super.testedStrategyIndex[currentGen-1]; 
		capabilities = super.capabilities[currentGen-1]; 
		probabilities = super.probabilities[currentGen-1]; 
		GaussProb = super.GaussProb[currentGen-1]; 
		GaussProb2 = super.GaussProb2[currentGen-1]; 
		changeIndexes = super.changeIndexes[currentGen-1]; 
		testStrategyIndexes = super.testStrategyIndexes[currentGen-1]; 
		capChangeRates = super.capChangeRates[currentGen-1]; 
		initialEnmity = super.initialEnmity[currentGen-1]; 
	...
	private static void simulation_initalize() {
		...
		implementClass_v24 implement = new implementClass_v24();
	...
	private static void initiateThreads(int operationID) {
		...
		implement = new implementClass_v24[processors];

		for (int threadID = 0; threadID < processors; threadID++) {
			implement[threadID] = new implementClass_v24(threadID, randNum, POPSIZE_PER_PROCESSOR,
