...
public class WorldSystem_v24 {
	...
	public WorldSystem_v24(int length, int[] testedStrategyIndex, double[] capabilities, double[] probabilities,
		...
		capChangeCount = 0;
		capChanges = GP_PilotStudy_v24.CAPCHANGE;
			...
			for (int i = 0; i < initialStatesNum; i++) {
				...
				if (surviving[i]) {
					if (strategy_init[i][capChangeCount][0] != null) {
						...
						strategy = strategy_init[i][capChangeCount][currentStatesNum - GP_PilotStudy_v24.MINSYSTEM];
						...
						for (int k = 0; k < initialStatesNum; k++) {
							...
							if (run() == 1) {
								...
								oppEnmity = 666;
								isStable = false;
								if (enmity[i][k] == GP_PilotStudy_v24.ENMITYLEVELS - 1) {
	...
	private double run() {
		...
		switch (primitive) {
		case GP_PilotStudy_v24.CAPMED:
			return (capMed);
		case GP_PilotStudy_v24.CAPAVG:
			return (capAvg);
		case GP_PilotStudy_v24.CAPSTD:
			return (capStd);
		case GP_PilotStudy_v24.CAPMIN:
			return (capMin);
		case GP_PilotStudy_v24.CAPMAX:
			return (capMax);
		case GP_PilotStudy_v24.MYCAP:
			return (myCap);
		case GP_PilotStudy_v24.OPPCAP:
			return (oppCap);
		case GP_PilotStudy_v24.MYSIDECAP:
			return (mySideCap);
		case GP_PilotStudy_v24.LEFTCAPSUM:
			return (leftCapSum);
		case GP_PilotStudy_v24.MYENMITY:
			return (myEnmity);
		case GP_PilotStudy_v24.OPPENMITY:
			return (oppEnmity);
		...
		default:
			return (GP_PilotStudy_v24.randNum[primitive]);
		}
