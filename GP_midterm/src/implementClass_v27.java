...
public class implementClass_v27 implements Runnable {
	double[] fitness;
	static double[] randNum;
	Random rd = new Random();
	int TESTCASES, threadID;

	static final GP_PilotStudy_v27 X = new GP_PilotStudy_v27();

	int gen, POPSIZE;
	public int operationID;
	boolean type2;

	char[][][] init_strategy, balance_strategy, bandwagon_strategy;
	
	public implementClass_v27(){
		
	}

	public implementClass_v27(int threadID, double[] randNum, int POPSIZE, double[] fitness, char[][][] init_strategy,
			char[][][] balance_strategy, char[][][] bandwagon_strategy, int TESTCASES, boolean type2) {
		...
		/*if (operationID == 1) {
			for (int k = 0; k < POPSIZE; k++) {
				for (int i = 0; i < X.uniqueWorldSizes - 1; i++) {
					...
					X.temp_balance_strategy[threadID][k][i] = create_random_indiv(X.DEPTH);
				}
	...
	void evolve() {
		...
		int[] parents;
		char[][] tempinit_strategy, tempbalance_strategy, tempbandwagon_strategy;
		char[] newind;
		double random;
		...
		balance_strategy=new char[X.balance_strategy.length-1][][];
		bandwagon_strategy=new char[X.bandwagon_strategy.length-1][][];
		System.arraycopy(X.init_strategy, 1, init_strategy, 0, X.init_strategy.length-1);
		System.arraycopy(X.balance_strategy, 1, balance_strategy, 0, X.balance_strategy.length-1);
		System.arraycopy(X.bandwagon_strategy, 1, bandwagon_strategy, 0, X.bandwagon_strategy.length-1);
		fitness = X.fitness;

		tempinit_strategy = new char[POPSIZE][];
		tempbalance_strategy = null;
		tempbandwagon_strategy = null;
		if (X.currentTestedSize > 2){
			tempbalance_strategy = new char[POPSIZE][];
			tempbandwagon_strategy = new char[POPSIZE][];
		}
		...
		for (indivs = 0; indivs < mod_crossovers; indivs++) {
				...
				tempbalance_strategy[indivs] = balance_strategy[tournament(fitness, X.TSIZE)][X.currentTestedSize - X.MINSYSTEM - 1];
				tempbandwagon_strategy[indivs] = bandwagon_strategy[tournament(fitness, X.TSIZE)][X.currentTestedSize - X.MINSYSTEM - 1];
		...
		if (X.currentTestedSize > 2) {
			...
			for (indivs = mod_crossovers; indivs < POPSIZE; indivs++) {
				...
				if (random < X.REPLICATION_PROB) {
					parent = tournament(fitness, X.TSIZE);
					tempbalance_strategy[indivs] = balance_strategy[parent][i];
				} else if ((random - X.REPLICATION_PROB) < X.CROSSOVER_PROB) {
					parent1 = tournament(fitness, X.TSIZE);
					parent2 = tournament(fitness, X.TSIZE);
					newind = crossover(balance_strategy[parent1][i], balance_strategy[parent2][i]);
					tempbalance_strategy[indivs] = newind;
				} else if ((random - (X.CROSSOVER_PROB + X.REPLICATION_PROB)) < X.SUBTREE_MUT_PROB) {
					parent = tournament(fitness, X.TSIZE);
					newind = subtreeMutation(balance_strategy[parent][i], type2);
					tempbalance_strategy[indivs] = newind;
				} else if ((random - (X.CROSSOVER_PROB + X.REPLICATION_PROB + X.SUBTREE_MUT_PROB)) < X.PMUT_PROB) {
					parent = tournament(fitness, X.TSIZE);
					newind = pointMutation(balance_strategy[parent][i], X.PMUT_PER_NODE, type2);
					tempbalance_strategy[indivs] = newind;
				} else {
					tempbalance_strategy[indivs] = create_random_indiv(X.DEPTH, type2);
				}
			}
			...
			for (indivs = mod_crossovers; indivs < POPSIZE; indivs++) {
				...
				if (random < X.REPLICATION_PROB) {
					parent = tournament(fitness, X.TSIZE);
					tempbandwagon_strategy[indivs] = bandwagon_strategy[parent][i];
				} else if ((random - X.REPLICATION_PROB) < X.CROSSOVER_PROB) {
					parent1 = tournament(fitness, X.TSIZE);
					parent2 = tournament(fitness, X.TSIZE);
					newind = crossover(bandwagon_strategy[parent1][i], bandwagon_strategy[parent2][i]);
					tempbandwagon_strategy[indivs] = newind;
				} else if ((random - (X.CROSSOVER_PROB + X.REPLICATION_PROB)) < X.SUBTREE_MUT_PROB) {
					parent = tournament(fitness, X.TSIZE);
					newind = subtreeMutation(bandwagon_strategy[parent][i], type2);
					tempbandwagon_strategy[indivs] = newind;
				} else if ((random - (X.CROSSOVER_PROB + X.REPLICATION_PROB + X.SUBTREE_MUT_PROB)) < X.PMUT_PROB) {
					parent = tournament(fitness, X.TSIZE);
					newind = pointMutation(bandwagon_strategy[parent][i], X.PMUT_PER_NODE, type2);
					tempbandwagon_strategy[indivs] = newind;
				} else {
					tempbandwagon_strategy[indivs] = create_random_indiv(X.DEPTH, type2);
				}
			}
		}
		...
		for (indivs = 0; indivs < POPSIZE; indivs++) {
			...
			if (X.currentTestedSize > 2){
				X.temp_balance_strategy[threadID][indivs][X.currentTestedSize - X.MINSYSTEM - 1] = (char[]) tempbalance_strategy[indivs].clone();
				X.temp_bandwagon_strategy[threadID][indivs][X.currentTestedSize - X.MINSYSTEM - 1] = (char[]) tempbandwagon_strategy[indivs].clone();
			}
		}
	}
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			balance_strategy = new char[length][X.CAPCHANGE + 1][X.uniqueWorldSizes - 1][];
			bandwagon_strategy = new char[length][X.CAPCHANGE + 1][X.uniqueWorldSizes - 1][];
			...
			for (int m = 0; m <= X.CAPCHANGE; m++) {
				for (int i = 0; i < length; i++) {
					...
					if (t < X.currentTestedSize) {
						for (int j = 0; j < X.currentTestedSize - 1; j++) {
							init_strategy[i][m][j] = (char[]) X.FT_init_strategy[X.currentTestedSize - X.MINSYSTEM][t][j]
									.clone();
						}
						if (X.currentTestedSize > 2)
							for (int j = 0; j < X.currentTestedSize - 2; j++){
								balance_strategy[i][m][j] = (char[]) X.FT_balance_strategy[X.currentTestedSize
										- X.MINSYSTEM - 1][t][j].clone();
								bandwagon_strategy[i][m][j] = (char[]) X.FT_bandwagon_strategy[X.currentTestedSize
								                   										- X.MINSYSTEM - 1][t][j].clone();
							}
						...
						/*for (int f = X.currentTestedSize - X.MINSYSTEM - 1; f >= 0; f--) {
							...
							if (f > 0)
								balance_strategy[i][m][f - 1] = (char[]) X.FT_balance_strategy[f - 1][rd
										.nextInt(X.FT_balance_strategy[f - 1].length - 1)].clone();
			...
			for (int i = 0; i < POPSIZE; i++) {		
				/*for (int m = 0; m <= X.CAPCHANGE; m++) {
					...
					for (int k = 1; k <= X.currentTestedSize - X.MINSYSTEM; k++) {
						if (this.init_strategy[i][k] != null) {
							...
							balance_strategy[testedStrategyIndex[m]][m][k - 1] = (char[]) this.balance_strategy[i][k - 1]
									.clone();
						}
					}
				}*/
				...
				for (int m = 0; m <= X.CAPCHANGE; m++) {
					...
					balance_strategy[testedStrategyIndex[m]][m] = this.balance_strategy[i];
					bandwagon_strategy[testedStrategyIndex[m]][m] = this.bandwagon_strategy[i];
				}
				
				

				world = new WorldSystem_v27(length, testedStrategyIndex, capabilities, probabilities, GaussProb, changeIndexes, capChangeRates, initialEnmity);
				boolean survived = world.simulate(init_strategy, balance_strategy, bandwagon_strategy);
			...
			if (rd.nextDouble() < pmut) {
				...
				if (parentcopy[mutsite] < X.FSET_1_START) {
					...
					case GP_PilotStudy_v27.EQ:
						...
						break;
					case GP_PilotStudy_v27.AND:
						parentcopy[mutsite] = GP_PilotStudy_v27.OR;
						break;
					case GP_PilotStudy_v27.OR:
						parentcopy[mutsite] = GP_PilotStudy_v27.AND;
						break;
					}
			}
		}
