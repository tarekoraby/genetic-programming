...
public class WorldSystem_v25 {
	...
	public WorldSystem_v25(int length, int[] testedStrategyIndex, double[] capabilities, double[] probabilities,
		...
		capChangeCount = 0;
		capChanges = GP_PilotStudy_v25.CAPCHANGE;
			...
			for (int i = 0; i < initialStatesNum; i++) {
				...
				if (surviving[i]) {
					if (strategy_init[i][capChangeCount][0] != null) {
						...
						strategy = strategy_init[i][capChangeCount][currentStatesNum - GP_PilotStudy_v25.MINSYSTEM];
						...
						for (int k = 0; k < initialStatesNum; k++) {
							...
							if (run() == 1) {
								...
								oppEnmity = 666;
								isStable = false;
								if (enmity[i][k] == GP_PilotStudy_v25.ENMITYLEVELS - 1) {
	...
	private double run() {
		...
		switch (primitive) {
		case GP_PilotStudy_v25.CAPMED:
			return (capMed);
		case GP_PilotStudy_v25.CAPAVG:
			return (capAvg);
		case GP_PilotStudy_v25.CAPSTD:
			return (capStd);
		case GP_PilotStudy_v25.CAPMIN:
			return (capMin);
		case GP_PilotStudy_v25.CAPMAX:
			return (capMax);
		case GP_PilotStudy_v25.MYCAP:
			return (myCap);
		case GP_PilotStudy_v25.OPPCAP:
			return (oppCap);
		case GP_PilotStudy_v25.MYSIDECAP:
			return (mySideCap);
		case GP_PilotStudy_v25.LEFTCAPSUM:
			return (leftCapSum);
		case GP_PilotStudy_v25.MYENMITY:
			return (myEnmity);
		case GP_PilotStudy_v25.OPPENMITY:
			return (oppEnmity);
		case GP_PilotStudy_v25.GT:
			if (run() > run())
				return (1);
			else
				return (0);
		case GP_PilotStudy_v25.LT:
			if (run() < run())
				return (1);
			else
				return (0);
		case GP_PilotStudy_v25.EQ:
			if (Math.abs(run() - run()) < 1E-10)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v25.AND:
			if (run() == 1 && run() == 1)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v25.OR:
			if (run() == 1 || run() == 1)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v25.ADD:
			return (run() + run());
		case GP_PilotStudy_v25.SUB:
			return (run() - run());
		case GP_PilotStudy_v25.MUL:
			return (run() * run());
		case GP_PilotStudy_v25.DIV: {
		...
		default:
			return (GP_PilotStudy_v25.randNum[primitive]);
		}
