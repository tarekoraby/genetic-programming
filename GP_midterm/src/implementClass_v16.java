...
public class implementClass_v16 implements Runnable {
	double[] fitness;
	static double[] randNum;
	Random rd = new Random();
	int TESTCASES, threadID;

	static final GP_PilotStudy_v16 X=new GP_PilotStudy_v16();
	...
	public implementClass_v16(int threadID, double[] randNum, int POPSIZE, double[] fitness,
	...
	public void run() {
		...
		if (operationID == 1) {
			for (int k = 0; k < POPSIZE; k++) {
				X.temp_init_strategy[threadID][k] = create_random_indiv(X.DEPTH);
				X.temp_join_strategy[threadID][k] = create_random_indiv(X.DEPTH);
			}
	...
	char[] grow(int depth, boolean returnBoolean) {
		...
		if (returnBoolean) {
			...
			if (prim == 0 || depth == 1) {
				...
				switch (prim) {
				case GP_PilotStudy_v16.GT:
				case GP_PilotStudy_v16.LT:
				case GP_PilotStudy_v16.EQ:
					buffer[0] = prim;
				...
				switch (prim) {
				case GP_PilotStudy_v16.AND:
				case GP_PilotStudy_v16.OR:
					buffer[0] = prim;
					System.arraycopy(leftBuffer, 0, buffer, 1, leftBuffer.length);
					System.arraycopy(rightBuffer, 0, buffer, (1 + leftBuffer.length), rightBuffer.length);
				}
			...
			if (prim == 0 || depth == 0) {
				...
				switch (prim) {
				case GP_PilotStudy_v16.ADD:
				case GP_PilotStudy_v16.SUB:
				case GP_PilotStudy_v16.MUL:
				case GP_PilotStudy_v16.DIV:
					buffer[0] = prim;
					System.arraycopy(leftBuffer, 0, buffer, 1, leftBuffer.length);
					System.arraycopy(rightBuffer, 0, buffer, (1 + leftBuffer.length), rightBuffer.length);
				}
	...
	void evolve() {
		...
		for (indivs = 0; indivs < mod_crossovers; indivs++) {
			tempinit_strategy[indivs] = init_strategy[tournament(fitness, X.TSIZE)];
			tempjoin_strategy[indivs] = join_strategy[tournament(fitness, X.TSIZE)];
		}
		...
		for (indivs = mod_crossovers; indivs < POPSIZE; indivs++) {
			...
			if (random < X.REPLICATION_PROB) {
				...
				newind = crossover(init_strategy[parent1], init_strategy[parent2]);
				...
				parent = tournament(fitness, X.TSIZE);
				newind = pointMutation(init_strategy[parent], X.PMUT_PER_NODE);
				tempinit_strategy[indivs] = newind;
			} else {
			...
			if (random < X.REPLICATION_PROB) {
				...
				parent1 = tournament(fitness, X.TSIZE);
				parent2 = tournament(fitness, X.TSIZE);
				newind = crossover(join_strategy[parent1], join_strategy[parent2]);
				...
				parent = tournament(fitness, X.TSIZE);
				newind = pointMutation(join_strategy[parent], X.PMUT_PER_NODE);
				tempjoin_strategy[indivs] = newind;
			} else {
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			for (int i = 0; i < POPSIZE; i++) {
				world = new WorldSystem_v16(length, testedStrategyIndex, capabilities, probabilities);
			...
			if (rd.nextDouble() < pmut) {
				...
				if (parentcopy[mutsite] < X.FSET_1_START) {
					...
					case GP_PilotStudy_v16.EQ:
						...
						break;
					case GP_PilotStudy_v16.AND:
						parentcopy[mutsite]=GP_PilotStudy_v16.OR;
						break;
					case GP_PilotStudy_v16.OR:
						parentcopy[mutsite]=GP_PilotStudy_v16.AND;
						break;
					}
			}
		}
