...
public class Simulation_v22 extends GP_PilotStudy_v22 {

	public Simulation_v22() {
			...
			for (currentTestedSize = MINSYSTEM; currentTestedSize <= MAXSYSTEM; currentTestedSize++) {
				...
				for (currentDeme = 0; currentDeme < currentTestedSize; currentDeme++) {
					...
					for (int demeGen = 0; demeGen < DEME_GENERATIONS; demeGen++) {
						evolve();
						temp_init_strategy[0][0][currentTestedIndex] = (char[]) FT_init_strategy[currentTestedIndex][currentDeme]
								.clone();
						if (currentTestedIndex > 0)
							temp_join_strategy[0][0][currentTestedIndex - 1] = (char[]) FT_join_strategy[currentTestedIndex - 1][currentDeme]
									.clone();
	...
	private void initializeDeme() {
		...
		for (int i = currentTestedSize - MINSYSTEM - 1; i >= 0; i--) {
			for (int indivs = 0; indivs < TOTAL_POPSIZE; indivs++) {
				init_strategy[indivs][i] = (char[]) FT_init_strategy[i][rd.nextInt(FT_init_strategy[i].length - 1)]
						.clone();
			}
			if (i > 0)
				for (int indivs = 0; indivs < TOTAL_POPSIZE; indivs++) {
					join_strategy[indivs][i - 1] = (char[]) FT_join_strategy[i - 1][rd
							.nextInt(FT_join_strategy[i - 1].length - 1)].clone();
				}
		}	
	}
	...
	private static void simulation_initalize() {
		...
		implementClass_v22 implement = new implementClass_v22();
	...
	private static void printStats(char[][][] init_strategy, char[][][] join_strategy) {
		...
		for (int i = 0; i < uniqueWorldSizes; i++) {
			...
			for (int k = 0; k < temp_init[i].length; k++)
				temp_init[i][k] = (char[]) init_strategy[i][k].clone();
		}
		...
		if (join_strategy != null) {
			...
			for (int i = 0; i < uniqueWorldSizes - 1; i++) {
				...
				for (int k = 0; k < temp_join[i].length; k++)
					temp_join[i][k] = (char[]) join_strategy[i][k].clone();
			}
	...
	private static void printProfilesToFile() {
		...
		if (PRINT_EXTENDED_PROFILES == true) {
			String outputString = "";
			String profile_id;
			for (int i = 0; i < attackProfiles.length; i++) {
				for (int k = 0; k < attackProfiles[i].length; k++) {
					for (int l = 0; l < PROFILES_PER_CAT; l++) {
						profile_id = (i + MINSYSTEM) + "0" + l;
						outputString += (currentSimulation + "," + (k + 1) + "," + currentGen + "," + (i + MINSYSTEM)
								+ "," + "0" + "," + (attackProfiles[i][k][l] == true ? 1 : 0) + "," + l + "\n");
					}

					if (i > 0) {
						for (int l = 0; l < PROFILES_PER_CAT; l++) {
							profile_id = (i + MINSYSTEM) + "1" + l;
							outputString += (currentSimulation + "," + (k + 1) + "," + currentGen + ","
									+ (i + MINSYSTEM) + "," + "1" + ","
									+ (balanceProfiles[i - 1][k][l] == true ? 1 : 0) + "," + l + "\n");
						}
						for (int l = 0; l < PROFILES_PER_CAT; l++) {
							profile_id = (i + MINSYSTEM) + "1" + l;
							outputString += (currentSimulation + "," + (k + 1) + "," + currentGen + ","
									+ (i + MINSYSTEM) + "," + "2" + ","
									+ (bandwagonProfiles[i - 1][k][l] == true ? 1 : 0) + "," + l + "\n");
						}
					}
				}
			}
	...
	private static void initiateThreads(int operationID) {
		...
		implement = new implementClass_v22[processors];

		for (int threadID = 0; threadID < processors; threadID++) {
			implement[threadID] = new implementClass_v22(threadID, randNum, POPSIZE_PER_PROCESSOR,
	...
	static int print_indiv(char[] buffer, int buffercounter) {
		...
		if (buffer[buffercounter] < FSET_1_START) {
			...
			case LEFTCAPSUM:
				...
				break;
			case MYENMITY:
				System.out.print("MY_ENMITY");
				break;
			case OPPENMITY:
				System.out.print("OPP_ENMITY");
				break;
			default:
