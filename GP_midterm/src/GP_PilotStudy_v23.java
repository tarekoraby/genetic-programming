...
public class GP_PilotStudy_v23 {
	...
	static World_System_Shell_v23[][] profilingWorlds_Attacks, profilingWorlds_Joins;
	...
	public static void main(String[] args) {
		...
		for (currentSimulation = 1; currentSimulation <= simulations; currentSimulation++) {
			Simulation_v23 simulation_v23 = new Simulation_v23();
		}
	...
	private static void master_initalize() {
		...
		profilingWorlds_Attacks = new World_System_Shell_v23[uniqueWorldSizes][PROFILES_PER_CAT];
		profilingWorlds_Joins = new World_System_Shell_v23[uniqueWorldSizes - 1][PROFILES_PER_CAT];
		...
		for (int i = 0; i < uniqueWorldSizes; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Attacks[i][k] = new World_System_Shell_v23(false, (i + MINSYSTEM));
			...
			if (i > 0)
				for (int k = 0; k < PROFILES_PER_CAT; k++) {
					profilingWorlds_Joins[i - 1][k] = new World_System_Shell_v23(true, (i + MINSYSTEM));
