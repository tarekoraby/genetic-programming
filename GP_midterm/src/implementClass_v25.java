...
public class implementClass_v25 implements Runnable {
	double[] fitness;
	static double[] randNum;
	Random rd = new Random();
	int TESTCASES, threadID;

	static final GP_PilotStudy_v25 X = new GP_PilotStudy_v25();
	...
	public implementClass_v25(){
		
	}

	public implementClass_v25(int threadID, double[] randNum, int POPSIZE, double[] fitness, char[][][] init_strategy,
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			for (int i = 0; i < POPSIZE; i++) {		
				...
				world = new WorldSystem_v25(length, testedStrategyIndex, capabilities, probabilities, GaussProb, changeIndexes, capChangeRates, initialEnmity);
			...
			if (rd.nextDouble() < pmut) {
				...
				if (parentcopy[mutsite] < X.FSET_1_START) {
					...
					case GP_PilotStudy_v25.EQ:
						...
						break;
					case GP_PilotStudy_v25.AND:
						parentcopy[mutsite] = GP_PilotStudy_v25.OR;
						break;
					case GP_PilotStudy_v25.OR:
						parentcopy[mutsite] = GP_PilotStudy_v25.AND;
						break;
					}
			}
		}
