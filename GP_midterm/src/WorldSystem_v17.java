...
public class WorldSystem_v17 {
	...
	public WorldSystem_v17(int length, int testedStrategyIndex, double[] capabilities, double[] probabilities, double[] GaussProb,
			int[][] changeIndexes, double[][] capChangeRates) {
		...
		capChangeCount = 0;
		capChanges = GP_PilotStudy_v17.CAPCHANGE;
			...
			for (int i = 0; i < initialStatesNum; i++) {
				...
				if (surviving[i]) {
					if (strategy_init[i][capChangeCount][0] != null) {
						...
						strategy = strategy_init[i][capChangeCount][currentStatesNum - GP_PilotStudy_v17.MINSYSTEM];
								...
								findJoiners: do {
									...
									for (int j = 0; j < initialStatesNum; j++) {
										...
										if (strategy_init[j][capChangeCount][0] == null) {
											...
											strategy = strategy_join[j][capChangeCount][currentStatesNum - GP_PilotStudy_v17.MINSYSTEM
													- 1];
											int rand;
											if (probabilities[counter++] > probabilities[counter++]) {
												rand = 0;
