...
public class implementClass_v18 implements Runnable {
	double[] fitness;
	static double[] randNum;
	Random rd = new Random();
	int TESTCASES, threadID;

	static final GP_PilotStudy_v18 X = new GP_PilotStudy_v18();
	...
	public implementClass_v18(int threadID, double[] randNum, int POPSIZE, double[] fitness, char[][][] init_strategy,
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			for (int m = 0; m <= X.CAPCHANGE; m++) {
				for (int i = 0; i < length; i++) {
					if (X.FT_init_strategy[testStrategyIndexes[m][i]][0] != null) {
						init_strategy[i][m][0] = (char[]) X.FT_init_strategy[testStrategyIndexes[m][i]][0].clone();
						for (int k = 1; k < X.uniqueWorldSizes; k++) {
							if (X.FT_init_strategy[testStrategyIndexes[m][i]][k] != null) {
								init_strategy[i][m][k] = (char[]) X.FT_init_strategy[testStrategyIndexes[m][i]][k]
										.clone();
								join_strategy[i][m][k - 1] = (char[]) X.FT_join_strategy[testStrategyIndexes[m][i]][k - 1]
										.clone();
							}
						}
					}
			...
			for (int i = 0; i < POPSIZE; i++) {		
				init_strategy[testedStrategyIndex][0][0] = (char[]) this.init_strategy[i][0].clone();
				for (int k = 1; k < X.uniqueWorldSizes; k++) {
					if (this.init_strategy[i][k] != null) {
						init_strategy[testedStrategyIndex][0][k] = (char[]) this.init_strategy[i][k].clone();
						join_strategy[testedStrategyIndex][0][k - 1] = (char[]) this.join_strategy[i][k - 1].clone();
					}
				}

				world = new WorldSystem_v18(length, testedStrategyIndex, capabilities, probabilities, GaussProb,
						changeIndexes, capChangeRates);
			...
			if (rd.nextDouble() < pmut) {
				...
				if (parentcopy[mutsite] < X.FSET_1_START) {
					...
					case GP_PilotStudy_v18.EQ:
						...
						break;
					case GP_PilotStudy_v18.AND:
						parentcopy[mutsite] = GP_PilotStudy_v18.OR;
						break;
					case GP_PilotStudy_v18.OR:
						parentcopy[mutsite] = GP_PilotStudy_v18.AND;
						break;
					}
			}
		}
