...
public class WorldSystem_v26 {
	...
	public WorldSystem_v26(int length, int[] testedStrategyIndex, double[] capabilities, double[] probabilities,
		...
		capChangeCount = 0;
		capChanges = GP_PilotStudy_v26.CAPCHANGE;
			...
			for (int i = 0; i < initialStatesNum; i++) {
				...
				if (surviving[i]) {
					if (strategy_init[i][capChangeCount][0] != null) {
						...
						strategy = strategy_init[i][capChangeCount][currentStatesNum - GP_PilotStudy_v26.MINSYSTEM];
						...
						for (int k = 0; k < initialStatesNum; k++) {
							...
							if (run() == 1) {
								...
								oppEnmity = 666;
								isStable = false;
								if (enmity[i][k] == GP_PilotStudy_v26.ENMITYLEVELS - 1) {
	...
	private double run() {
		...
		switch (primitive) {
		case GP_PilotStudy_v26.CAPMED:
			return (capMed);
		case GP_PilotStudy_v26.CAPAVG:
			return (capAvg);
		case GP_PilotStudy_v26.CAPSTD:
			return (capStd);
		case GP_PilotStudy_v26.CAPMIN:
			return (capMin);
		case GP_PilotStudy_v26.CAPMAX:
			return (capMax);
		case GP_PilotStudy_v26.MYCAP:
			return (myCap);
		case GP_PilotStudy_v26.OPPCAP:
			return (oppCap);
		case GP_PilotStudy_v26.MYSIDECAP:
			return (mySideCap);
		case GP_PilotStudy_v26.LEFTCAPSUM:
			return (leftCapSum);
		case GP_PilotStudy_v26.MYENMITY:
			return (myEnmity);
		case GP_PilotStudy_v26.OPPENMITY:
			return (oppEnmity);
		case GP_PilotStudy_v26.GT:
			if (run() > run())
				return (1);
			else
				return (0);
		case GP_PilotStudy_v26.LT:
			if (run() < run())
				return (1);
			else
				return (0);
		case GP_PilotStudy_v26.EQ:
			if (Math.abs(run() - run()) < 1E-10)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v26.AND:
			if (run() == 1 && run() == 1)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v26.OR:
			if (run() == 1 || run() == 1)
				return (1);
			else
				return (0);
		case GP_PilotStudy_v26.ADD:
			return (run() + run());
		case GP_PilotStudy_v26.SUB:
			return (run() - run());
		case GP_PilotStudy_v26.MUL:
			return (run() * run());
		case GP_PilotStudy_v26.DIV: {
		...
		default:
			return (GP_PilotStudy_v26.randNum[primitive]);
		}
