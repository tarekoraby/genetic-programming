...
public class implementClass_v19 implements Runnable {
	double[] fitness;
	static double[] randNum;
	Random rd = new Random();
	int TESTCASES, threadID;

	static final GP_PilotStudy_v19 X = new GP_PilotStudy_v19();
	...
	public implementClass_v19(int threadID, double[] randNum, int POPSIZE, double[] fitness, char[][][] init_strategy,
	...
	char[] grow(int depth, boolean returnBoolean) {
		...
		if (returnBoolean) {
			...
			if (prim == 0 || depth == 1) {
				...
				switch (prim) {
				case GP_PilotStudy_v19.GT:
				case GP_PilotStudy_v19.LT:
				case GP_PilotStudy_v19.EQ:
					buffer[0] = prim;
				...
				switch (prim) {
				case GP_PilotStudy_v19.AND:
				case GP_PilotStudy_v19.OR:
					buffer[0] = prim;
					System.arraycopy(leftBuffer, 0, buffer, 1, leftBuffer.length);
					System.arraycopy(rightBuffer, 0, buffer, (1 + leftBuffer.length), rightBuffer.length);
				}
			...
			if (prim == 0 || depth == 0) {
				...
				switch (prim) {
				case GP_PilotStudy_v19.ADD:
				case GP_PilotStudy_v19.SUB:
				case GP_PilotStudy_v19.MUL:
				case GP_PilotStudy_v19.DIV:
					buffer[0] = prim;
					System.arraycopy(leftBuffer, 0, buffer, 1, leftBuffer.length);
					System.arraycopy(rightBuffer, 0, buffer, (1 + leftBuffer.length), rightBuffer.length);
				}
	...
	double[] calculateFitness(int testCases) {
		...
		for (int test = 0; test < testCases; test++) {
			...
			for (int i = 0; i < POPSIZE; i++) {		
				...
				world = new WorldSystem_v19(length, testedStrategyIndex, capabilities, probabilities, GaussProb,
			...
			if (rd.nextDouble() < pmut) {
				...
				if (parentcopy[mutsite] < X.FSET_1_START) {
					...
					case GP_PilotStudy_v19.EQ:
						...
						break;
					case GP_PilotStudy_v19.AND:
						parentcopy[mutsite] = GP_PilotStudy_v19.OR;
						break;
					case GP_PilotStudy_v19.OR:
						parentcopy[mutsite] = GP_PilotStudy_v19.AND;
						break;
					}
			}
		}
