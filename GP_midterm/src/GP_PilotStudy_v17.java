...
public class GP_PilotStudy_v17 {
	public static final boolean PROFILEPOWER = false;
	static int DEMES = 3, MASTER_GENERATIONS = 10000, DEME_GENERATIONS = 1, POPSIZE_PER_PROCESSOR = 100, MINRANDOM = 0,
			MAXRANDOM = 1, PROFILES_PER_CAT = 1000, MINSYSTEM = 2, MAXSYSTEM = 3, TESTCASES = 2000, MAX_LEN = 10000,
			DEPTH = 5, TSIZE = 2, simulations = 1, stabilityNum = 1000;
	...
	public static final double RANDOMPLAYER_PROB = 0.1, PMUT_PER_NODE = 0.2, MOD_CROSSOVER_PROB = 0.3,
			REPLICATION_PROB = 0, CROSSOVER_PROB = 0, SUBTREE_MUT_PROB = 0, PMUT_PROB = 0.2, ABIOGENSIS_PROB = 0.5,
			GAUSS_PROB_FACTOR = 0;
	static final int CAPCHANGE = 1;
	static int currentGen, uniqueWorldSizes, currentSimulation;
	static implementClass_v17[] implement;
	...
	static World_System_Shell_v17[][] profilingWorlds_Attacks, profilingWorlds_Joins;
	...
	static int[] length, testedStrategyIndex, world_sizes;
	...
	static WriteFile writer, stateStats;
	...
	public static void main(String[] args) {
		...
		for (currentSimulation = 1; currentSimulation <= simulations; currentSimulation++) {
			...
			for (currentGen = 1; currentGen <= MASTER_GENERATIONS; currentGen++) {
				System.out.println("Working on Deme");
				for (int currentDeme = 0; currentDeme < DEMES; currentDeme++) {
					...
					if (prevBestFitness != fitness[0]) {
						...
						FT_init_strategy[currentDeme][uniqueWorldSizes - 1] = (char[]) init_strategy[0][uniqueWorldSizes - 1]
								.clone();
					} else {
						if (FT_init_strategy[currentDeme][0] != null) {
							for (int i = 0; i < uniqueWorldSizes - 1; i++) {
				...
				if (prevSimilarityCount == stabilityNum) {
					System.out.println("System reached stability !!!!!!!");
					break;
	...
	private static void printStats(char[][][] init_strategy, char[][][] join_strategy) {
		...
		for (int i = 0; i < length; i++) {
			if (init_strategy[i][0] != null && join_strategy[i][0] != null) {
				validStrategies++;
			}
		}
		...
		if (totalAttacks == prevAttacks && totalJoins == prevJoins)
			prevSimilarityCount++;
		else {
			prevAttacks = totalAttacks;
			prevJoins = totalJoins;
			...
			for (int i = 0; i < length; i++) {
				for (int k = 1; k < uniqueWorldSizes; k++) {
					stateStats.writeToFile(currentSimulation + "," + (i + 1) + "," + currentGen + ","
							...
							+ indiv_balances[i][k - 1] + ","
							+ indiv_bandwagons[i][k - 1]);
				}
			}
	...
	private static void master_initalize() {
		...
		try {
			stateStats = new WriteFile("stateStats.txt", false);
			stateStats.writeToFile("currentSimulation" + "," + "stateNum" + "," + "generation" + "," + "gen_att_perc"
					+ "," + "gen_join_perc" + "," + "totalAttacks" + "," + "totalJoins" + "," + "world_attacks" + ","
					+ "world_joins" + "," + "world_size" + "," + "indiv_attcks" + "," + "indiv_joins" + ","
					+ "indiv_balances" + "," + "indiv_bandwagones");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		GaussProb = new double[TESTCASES][];
		changeIndexes = new int[TESTCASES][CAPCHANGE][];
		testStrategyIndexes = new int[TESTCASES][CAPCHANGE + 1][];
		capChangeRates = new double[TESTCASES][CAPCHANGE][];
		...
		profilingWorlds_Attacks = new World_System_Shell_v17[uniqueWorldSizes][PROFILES_PER_CAT];
		profilingWorlds_Joins = new World_System_Shell_v17[uniqueWorldSizes - 1][PROFILES_PER_CAT];
		...
		for (int i = 0; i < uniqueWorldSizes; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Attacks[i][k] = new World_System_Shell_v17(false, (i + MINSYSTEM));
		...
		for (int i = 0; i < uniqueWorldSizes - 1; i++) {
			for (int k = 0; k < PROFILES_PER_CAT; k++) {
				profilingWorlds_Joins[i][k] = new World_System_Shell_v17(true, i + MINSYSTEM + 1);
		...
		for (int test = 0; test < TESTCASES; test++) {
			...
			int difference = MAXSYSTEM - length[test];
			for (int i = 0; i < difference; i++) {
				length[test] = MINSYSTEM + rd.nextInt(MAXSYSTEM - MINSYSTEM + 1);
				difference = MAXSYSTEM - length[test];
			}
			...
			for (int k = 0; k < CAPCHANGE + 1; k++) {
				testStrategyIndexes[test][k] = new int[length[test]];
				for (int i = 0; i < length[test]; i++) {
					testStrategyIndexes[test][k][i] = rd.nextInt(FT_init_strategy.length - 1);
					if (rd.nextDouble() < RANDOMPLAYER_PROB)
						testStrategyIndexes[test][k][i] = FT_init_strategy.length - 1;
				}
			}
		...
		System.out.println("Among " + TESTCASES + " test cases there are");
		for (int i = 0; i < uniqueWorldSizes; i++) {
	...
	private static void initiateThreads(int operationID) {
		...
		implement = new implementClass_v17[processors];

		for (int threadID = 0; threadID < processors; threadID++) {
			implement[threadID] = new implementClass_v17(threadID, randNum, POPSIZE_PER_PROCESSOR,
					tempFitness[threadID], temp_init_strategy[threadID], temp_join_strategy[threadID], TESTCASES);
