...
public class GP_PilotStudy_v8 {
	double[] fitness;
	static double[] randNum;
	static char[][] strategy_init_2, strategy_init_3, strategy_init_4, strategy_join_3, FT_strategy_init_2,
	FT_strategy_init_3, FT_strategy_init_4, FT_strategy_join_3;
	static Random rd = new Random();
	static final int MAX_LEN = 10000, POPSIZE = 1000, DEPTH = 5, GENERATIONS = 10000, TSIZE = 2, BESTRETAINED = 100,
			TSIZE_2 = 10, MOD_CROSSOVERS = BESTRETAINED, TESTCASES = 1000, PROFILES = 10000,
			MINSYSTEM = 2, MAXSYSTEM = 3, RANDOMNUMBERS = 50, BLOATSTART=1000;
	static double MINRANDOM = 0, MAXRANDOM = 1;
	public static final double PMUT_PER_NODE = 0.05, REPLICATION_PROB = 0.3, CROSSOVER_PROB = 0.25,
			SUBTREE_MUT_PROB = 0.25, PMUT_PROB = 0.1, ABIOGENSIS_PROB = 0.1;
	...
	static final int CAPCHANGE = 4;
	World_System_Shell_v8[] profilingWorlds;
	boolean[][] profile;
	boolean[][] profile10;
	...
	public static void main(String[] args) {
		...
		GP_PilotStudy_v8 gp = new GP_PilotStudy_v8();
		gp.evolve();
		
	}

	public GP_PilotStudy_v8() {
		...
		profilingWorlds = new World_System_Shell_v8[PROFILES];
		for (int i = 0; i < PROFILES / 2; i++)
			profilingWorlds[i] = new World_System_Shell_v8(false);
		if (MAXSYSTEM > 2)
			for (int i = PROFILES / 2; i < PROFILES; i++)
				profilingWorlds[i] = new World_System_Shell_v8(true);
	...
	void evolve() { 
		...
		// for the first round take the first randomly generated strategies as the test strategies
		FT_strategy_init_2 = new char[BESTRETAINED][];
		FT_strategy_init_3 = new char[BESTRETAINED][];
		FT_strategy_init_4 = new char[BESTRETAINED][];
		FT_strategy_join_3 = new char[BESTRETAINED][];
		for (int i = 0; i < BESTRETAINED; i++) {
			FT_strategy_init_2[i] = strategy_init_2[i].clone();
			FT_strategy_init_3[i] = strategy_init_3[i].clone();
			FT_strategy_init_4[i] = strategy_init_4[i].clone();
			FT_strategy_join_3[i] = strategy_join_3[i].clone();
		}
		
		fitness = calculateFitness( TESTCASES);
		stats(  0 );
		...
		for (gen=0; gen <= GENERATIONS; gen++) {
			...
			if (sortedFit) {
				for (indivs = 0 ; indivs < BESTRETAINED ; indivs++) { 
					parent = tournament(fitness, TSIZE_2);
					FT_strategy_init_2[indivs] = tempstrategy_init_2[indivs] = strategy_init_2[parent].clone();
					FT_strategy_init_3[indivs] = tempstrategy_init_3[indivs] = strategy_init_3[parent].clone();
					FT_strategy_init_4[indivs] = tempstrategy_init_4[indivs] = strategy_init_4[parent].clone();
					FT_strategy_join_3[indivs] = tempstrategy_join_3[indivs] = strategy_join_3[parent].clone();
				}
			} else {
	...
	private void sortDesc(char[][] strategy_init_2, char[][] strategy_init_3, char[][] strategy_init_4, char[][] strategy_join_3, double[] fitness) {
		...
		for (int i = 0; i < POPSIZE; i++) {
			...
			tempstrategy_init_3[i] = strategy_init_3[i].clone();
			tempstrategy_init_4[i] = strategy_init_4[i].clone();
			tempstrategy_join_3[i] = strategy_join_3[i].clone();
		}
		...
		for (int i = 0; i < POPSIZE; i++) {
			...
			while (strategy_init_2[i] == null && j < POPSIZE) {
				if (fitness[i] == tempFitness[j]) {
					...
					strategy_init_3[i] = tempstrategy_init_3[j].clone();
					strategy_init_4[i] = tempstrategy_init_4[j].clone();
					strategy_join_3[i] = tempstrategy_join_3[j].clone();
		...
		GP_PilotStudy_v8.strategy_init_2 = strategy_init_2;
		GP_PilotStudy_v8.strategy_init_3 = strategy_init_3;
		GP_PilotStudy_v8.strategy_init_4 = strategy_init_4;
		GP_PilotStudy_v8.strategy_join_3 = strategy_join_3;
	...
	void stats(int gen) {
		...
		for (int i = 0; i < BESTRETAINED; i++) {
			favgpop += fitness[i];
			node_count_1 += traverse(FT_strategy_init_2[i], 0);
			node_count_2 += traverse(FT_strategy_join_3[i], 0);
		}
		favgpop /= BESTRETAINED;
		avg_len = (double) (node_count_1 + node_count_2) / POPSIZE; 
		
		calcSimilarity(FT_strategy_init_2, FT_strategy_init_3, FT_strategy_init_4, FT_strategy_join_3, BESTRETAINED);
		...
		if (PRINTINDIV) {
			...
			print_indiv(FT_strategy_init_2[0], 0);
			System.out.print("\nJoining strategy: ");
			print_indiv(FT_strategy_join_3[0], 0);
			System.out.println();
		}
	...
	double[] calculateFitness(int testCases) {
		fitness = new double[POPSIZE];
		int length, index;
		WorldSystem_v8 world;
		for (int i=0; i<POPSIZE; i++){
			for (int test = 0; test < testCases; test++){
				length = MINSYSTEM + rd.nextInt(MAXSYSTEM - MINSYSTEM + 1);
				char[][] test_strategy_init_2 = new char[length][];
				char[][] test_strategy_init_3 = new char[length][];
				char[][] test_strategy_init_4 = new char[length][];
				char[][] test_strategy_join_3 = new char[length][];
				test_strategy_init_2[0] = strategy_init_2[i].clone();
				test_strategy_init_3[0] = strategy_init_3[i].clone();
				test_strategy_init_4[0] = strategy_init_4[i].clone();
				test_strategy_join_3[0] = strategy_join_3[i].clone();

				
				for (int k = 1; k < length; k++) {
					index = rd.nextInt(BESTRETAINED);
					test_strategy_init_2[k] = FT_strategy_init_2[index].clone();
					test_strategy_init_3[k] = FT_strategy_init_3[index].clone();
					test_strategy_init_4[k] = FT_strategy_init_4[index].clone();
					test_strategy_join_3[k] = FT_strategy_join_3[index].clone();
				}

				world = new WorldSystem_v8(test_strategy_init_2, test_strategy_init_3, test_strategy_init_4, test_strategy_join_3, length);
