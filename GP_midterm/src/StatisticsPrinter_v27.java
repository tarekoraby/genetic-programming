...
public class StatisticsPrinter_v27 extends GP_PilotStudy_v27{
	...
	static void printStats(char[][][][] init_strategy, char[][][][] balance_strategy, char[][][][] bandwagon_strategy) {
		...
		char[][][] temp_balance = null;
		char[][][] temp_bandwagon = null;
		...
		if (balance_strategy != null) {
			temp_balance = new char[uniqueWorldSizes - 1][][];
			temp_bandwagon = new char[uniqueWorldSizes - 1][][];
			for (int i = 0; i < uniqueWorldSizes - 1; i++) {
				temp_balance[i] = new char[i + MINSYSTEM + 1][];
				temp_bandwagon[i] = new char[i + MINSYSTEM + 1][];
				for (int k = 0; k < temp_balance[i].length; k++){
					temp_balance[i][k] = (char[]) balance_strategy[i][k][i].clone();
					temp_bandwagon[i][k] = (char[]) bandwagon_strategy[i][k][i].clone();
				}
			}
		}
		

		populateProfiles(temp_init, temp_balance, temp_bandwagon);	
		...
		if (MAXSYSTEM > 2) {
			...
			for (int i = 0; i < bandwagonProfiles.length; i++) {
				System.out.println("In world " + (i + MINSYSTEM + 1) + ", percentage of buckpasses of state ");
				for (int k = 0; k < bandwagonProfiles[i].length; k++) {
					System.out.print((k + 1) + " is ");
					int totalbuckpasses = 0;
					for (int l = 0; l < PROFILES_PER_CAT; l++) {
						if (balanceProfiles[i][k][l] == false && bandwagonProfiles[i][k][l] == false)
							totalbuckpasses++;
					}
					System.out.println((double) totalbuckpasses / (PROFILES_PER_CAT) * 100 + " %");
	...
	private static void populateProfiles(char[][][] init_strategy, char[][][] balance_strategy, char[][][] bandwagon_strategy) {
		...
		if (balance_strategy != null) {
			balanceProfiles = new boolean[balance_strategy.length][][];
			bandwagonProfiles = new boolean[bandwagon_strategy.length][][];

			for (int i = 0; i < balanceProfiles.length; i++) {
				balanceProfiles[i] = new boolean[balance_strategy[i].length][PROFILES_PER_CAT];
				bandwagonProfiles[i] = new boolean[bandwagon_strategy[i].length][PROFILES_PER_CAT];
			}

			for (int i = 0; i < balanceProfiles.length; i++) {
				for (int k = 0; k < balanceProfiles[i].length; k++) {
					for (int l = 0; l < PROFILES_PER_CAT; l++) {
						balanceProfiles[i][k][l] = profilingWorlds_Joins[i][l].willItAttack(balance_strategy[i][k], 1);
						bandwagonProfiles[i][k][l] = profilingWorlds_Joins[i][l].willItAttack(bandwagon_strategy[i][k], 2);
					}
				}
			}
